// @ts-check
// Gate lint/format frontend (réserve R1, Gate 6). Config plate ESLint v9 + angular-eslint v20.
// Exécutée via `ng lint` (cible angular.json) et la CI (job frontend). Préfixe sélecteurs : `app`.
const eslint = require("@eslint/js");
const tseslint = require("typescript-eslint");
const angular = require("angular-eslint");

module.exports = tseslint.config(
  {
    files: ["**/*.ts"],
    extends: [
      eslint.configs.recommended,
      ...tseslint.configs.recommended,
      ...tseslint.configs.stylistic,
      ...angular.configs.tsRecommended,
    ],
    processor: angular.processInlineTemplates,
    rules: {
      "@angular-eslint/directive-selector": [
        "error",
        { type: "attribute", prefix: "app", style: "camelCase" },
      ],
      "@angular-eslint/component-selector": [
        "error",
        // "lt" : composants du Design System LoyerTracker (DSG-001.md §Naming Convention,
        // introduits par US-132) ; "app" : composants applicatifs pré-existants, conservés.
        { type: "element", prefix: ["app", "lt"], style: "kebab-case" },
      ],
    },
  },
  {
    files: ["**/*.html"],
    extends: [
      ...angular.configs.templateRecommended,
      ...angular.configs.templateAccessibility,
    ],
    rules: {},
  }
);
