...
| Champ | Valeur |
|---|---|
| Date initiale / dernière mise à jour | 2026-06-14 / 2026-07-04 |
| Branche de référence | `main` |
| Lot initial / dernier déploiement | Production Readiness 4b / S1 Gate Production `1.8.0` — Sprint 10 EP-12b + correctif RSV-S10-01 (PR #173) |
...
| `LOYERTRACKER_TAG` | **déployé : `sha-e4744d92`** (EP-16 Sprint N Fondation notifications, PR #235/#236, déployé le 2026-07-19, STG-ISOL-01 PASS avant/après, Flyway V27 27/27, smoke 63/0, 24 événements et 0 Outbox/Delivery — `docs/cgpa/07-devsecops/gate-staging-sprint-n-ep16-decision.md`) |
...
6. **Écart NPM — basic-auth bloquait la page publique de vérification `/verify/` (découvert le
   2026-07-24, Gate Staging Sprint N+1 EP-16).** Après confirmation par le PO de la réception
   réelle du message WhatsApp, l'ouverture du lien de vérification (`/verify/receipt/{id}`)
   demandait les identifiants basic-auth NPM — alors que le contrat de sécurité de cette page est
   d'être accessible **sans compte**, la seule preuve d'autorisation étant le token HMAC porté par
   l'URL (même conception que `/api/public/receipts/*`, déjà exemptée via le bypass `/api/`
   ci-dessus §5). Une bascule complète du basic-auth pour tout le domaine a été explicitement
   écartée (exposerait le reste du staging, environnement de test partagé, sans bénéfice).
   **Correctif ciblé** : ajout d'un second bloc `location /verify/ { auth_basic off; … }` dans
   `18.conf`, même patron exact que le bloc `/api/` existant, rien d'autre modifié. Persisté dans
   `advanced_config` (SQLite NPM) + rechargement `nginx -s reload` dans le conteneur NPM (pas de
   redémarrage du conteneur — aucun autre projet hébergé sur cet hôte affecté). Vérification live :
   `GET /verify/receipt/{id}` sans identifiants → **200** ✅ ; `GET /` (racine, reste protégée) sans
   identifiants → **401** ✅ ; 9 conteneurs inchangés (STG-ISOL-01 non re-décomposé, aucune
   commande Docker globale).
...
| 2026-07-04 | `sha-2c5f43c7` | **S1 — Gate Production `1.8.0` (RSV-S10-02)** : candidat Sprint 10 + correctif RSV-S10-01 (PR #173, merge `2c5f43c7`) | **9/9** | **59/0 (1er passage)** | — / — | Redéploiement ciblé décidé par arbitrage PO **S1** (Gate Production `1.8.0` §4.1). Sauvegarde pré-déploiement (`loyertracker-staging-20260704-153707.dump`, 368 Kio, 742 entrées). STG-ISOL-01 PASS avant/après (15:36/15:49 UTC, restart=0, NPM intact). Invocation canonique `-f docker-compose.staging.yml` seule — **aucun incident** (leçon Sprint 10 appliquée). `api`+`nginx` recréés ; Flyway inchangé **21/21** (aucune migration). Smoke **59/0 au premier passage**. **Vérification live RSV-S10-01** : scénario réel dépôt→retenue→complément le même jour — `GET /mouvements` et export CSV dans l'ordre chronologique ✅ ; **preuve discriminante** : l'ancien tri UUID aurait rendu `RETENUE_LOYER, DEPOT_INITIAL, COMPLEMENT` sur ces mêmes données ; liaison V21 vérifiée ; invariant 5/5 ; nettoyage transactionnel **0 résidu** (compteurs + audit à la baseline), échafaudage kcadm révoqué. Observation qualifiée OBS-S10-01 (cosmétique, lignes backfill V20 à `cree_le` identique — aucun cas en Production). **RSV-S10-02 LEVÉE — Gate Production `1.8.0` GO, `PRODUCTION_READY` atteint.** Décision : `gate-production-v1.8.0-decision.md` §8. |
| 2026-07-06 | `sha-9713fdaa` | Sprint 11 EP-14a Quittances certifiées (PR #183, merge `eddc037`) + Sprint Angular 20→22 (PR #197, merge `05a4f86`) + correctifs CI/doc (PR #194/#195/#196/#198) | **8/8** | **59/0** | — / — | Préflight : `QUITTANCE_HMAC_SECRET` généré (`openssl rand -hex 32`) et injecté au `.env` hôte (absent, défaut vide dans le compose — signature HMAC non fonctionnelle en l'état), `QUITTANCE_TOKEN_KID=1`, `QUITTANCE_VERIFY_BASE_URL` confirmée. `git pull --ff-only` `20d6701` → `9713fda` (58 fichiers). STG-ISOL-01 PASS avant/après (8 conteneurs `loyertracker-staging-*` + `nginx-proxy-manager` intact). `api`+`nginx` recréés ; `postgres`/`keycloak` non redémarrés. Flyway : **V22 appliquée** (`quittance` versionnée, `quittance_numerotation`, `quittance_verification_log`, RLS FORCE), total 22/22. Smoke **59/0**. **Vérification visuelle du rendu PDF certifié** (objectif spécifique du sprint) : tunnel SSH local + Chromium headless, flux OIDC/PKCE réel, adresse bailleur renseignée (« Mon profil »), quittance téléchargée et rendue — logo, badge DOCUMENT CERTIFIÉ, numéro `QT-2026-000001`, locataire anonymisé (RGPD), QR code + empreinte SHA-256, mentions légales — **conforme**. Le QR pointe vers `/verify/receipt/:id` (Sprint 12, page non encore livrée — attendu, ADR-15 K5). **Gate Staging Sprint 11 GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint11-ep14a-v5.4.1-decision.md`. **Promotion Production explicitement bloquée avant la release unique `1.9.0`** (ADR-15 K5). |
| 2026-07-06 | `sha-75646d8f` | Sprint 12 EP-14b Vérification publique des quittances (PR #202, merge `75646d8`) | **9/9** | **62/0 au premier passage** | métriques quittances présentes / 404 | Revue sécurité dédiée PASS. Sauvegarde `loyertracker-20260706-171755.dump` (517 Kio, 777 entrées, SHA-256 `62d7790f…`). STG-ISOL-01 PASS avant/après : NPM et 9 conteneurs intacts, restart=0, réseaux/volumes/ports inchangés. Déploiement ciblé `api`+`nginx` uniquement ; PostgreSQL/Keycloak/monitoring non recréés. Flyway 22/22. Rate-limit live : 21×200 puis 69×429. Parcours public réel `QT-2026-000001` VALIDE et PDF conforme à `pdf_hash`. **Gate Staging Sprint 12 GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint12-ep14b-v5.4.1-decision.md`. Production non autorisée ; Gate `1.9.0` distinct requis. |
| 2026-07-15 | `sha-c9200a51` | Sprints A+B EP-15 Gestion des personnes (PR #209, merge `4d8a760`, + PR #217 doc `c9200a5`) | **8/8** | **62/0 + 48 PASS/0 FAIL vérification manuelle** | — / — | Revue de code PR #209 : 2 bugs corrigés avant merge (photo Gestionnaire effacée silencieusement, `dateCreation` Locataire null), commit `66a57dc`. Revue sécurité dédiée PASS (`security-review-sprints-ab-ep15.md`, pattern cross-tenant `SECURITY DEFINER`). Sauvegarde pré-déploiement `loyertracker-staging-20260715-105158.dump` (531336 octets, 777 entrées, SHA-256 `6465f6d7…`). STG-ISOL-01 PASS avant/après : NPM et 8 conteneurs `loyertracker-staging-*` intacts, restart=0. Déploiement ciblé `api`+`nginx` uniquement ; PostgreSQL/Keycloak/monitoring non recréés. Flyway : **V23 (statut Gestionnaire) + V24 (Locataire) appliquées, total 24/24**. Smoke **62/0** (script non étendu aux nouveaux endpoints, comme garantie Sprint 9/10). **Vérification manuelle live dédiée (48 PASS/0 FAIL)** : CRUD/cycle de vie/recherche/doublons/historique Locataire, isolation RLS cross-tenant (404) ; profil/cycle de vie/RBAC ReBAC (403)/garde cross-tenant d'archivage (409→200 après révocation) Gestionnaire ; **les deux correctifs du PR #209 reconfirmés en conditions réelles** (`dateCreation` renseignée, photo conservée sur mise à jour partielle). **Gate Staging Sprints A+B GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprints-ab-ep15-decision.md`. Production non autorisée (pas de promotion isolée du Sprint A avant Sprint B, cf. plan d'exécution EP-15) ; Gate Production distinct requis. |
| 2026-07-16 | `sha-cba13f52` | Sprint unique EP-13 Fin de bail (PR #227, merge `cba13f5`) | **9/9** | **62/0 + 22 PASS/0 FAIL vérification manuelle** | — / — | CI post-merge 7/7 (correctif Sonar `java:S5778` nécessaire avant Quality Gate PASS). Sauvegarde pré-déploiement `loyertracker-20260716-172113.dump` (549489 octets, 798 entrées, SHA-256 `eaad8cda…`). STG-ISOL-01 PASS avant/après : NPM et 9 conteneurs `loyertracker-staging-*` intacts, restart=0. Déploiement ciblé `api`+`nginx` uniquement ; PostgreSQL/Keycloak/monitoring non recréés. Flyway : **V25 (`bail.date_cloture_effective` + `generer_alertes()` filtrée `ACTIF` sur `LOYER_EN_RETARD`) appliquée, total 25/25**. Smoke **62/0** (script non étendu aux nouveaux endpoints, comme Garantie Sprint 9/10 et Gestionnaire/Locataire Sprints A+B). **Vérification manuelle live dédiée (22 PASS/0 FAIL)** : clôture avec avertissements non bloquants (garantie non restituée + paiements en cours), `dateFin` contractuelle inchangée, purge des `A_VENIR` futurs (0 restant) sans toucher les `EN_RETARD` historiques (6 conservés), non-régression alertes confirmée (0 `LOYER_EN_RETARD` sur bail clos), clôture d'un bail déjà clos → 409, réouverture réussie, collision `uq_bail_actif` → 409. **Incident mineur signalé** : une commande de diagnostic (`set -x`) a exposé les 7 secrets `.env` Staging dans la session de travail — rotation recommandée, non bloquante pour ce Gate. **Gate Staging EP-13 GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-ep13-fin-de-bail-decision.md`. Production non autorisée ; Gate Production distinct requis. |
| 2026-07-17 | `sha-359f4d63` | Sprint C EP-15 Bascule Bail → Locataire (PR #229, merge `359f4d6`) | **8/8** | **63/0 (smoke étendu)** | — / — | CI PR #229 : correctif Sonar `java:S107` (constructeur `Bail`, 9 paramètres) pré-validé contre l'instance SonarQube réelle avant re-push — leçon appliquée. CI post-merge 4/4 verte, images GHCR publiées. Sauvegarde pré-déploiement `loyertracker-staging-20260717-075922.dump` (554520 octets, 799 entrées, SHA-256 `a197bd9c…`) + globals. STG-ISOL-01 PASS avant/après : NPM et 9 conteneurs `loyertracker-staging-*` intacts, restart=0. Déploiement ciblé `api`+`nginx` uniquement. Flyway : **V26 (bascule Bail→Locataire, non additive) appliquée, total 26/26**. **Backfill vérifié sur données réelles** : 35/35 baux avec `locataire_id` renseigné, 0 orphelin, `locataire_nom`/`locataire_email` supprimées, `prenom` non peuplé (RSV-EP15-02 confirmé). Smoke **63/0 au premier passage** (script étendu : création `Locataire` + bail via `locataireId`, nouvel endpoint RGPD `DELETE /api/locataires/{id}/effacement` — 403 gestionnaire, 204 bailleur, anonymisation reflétée dans l'historique/export). **Nettoyage transactionnel** : résidu accumulé sur plusieurs sessions passées sur `bailleur-test` (jamais nettoyé) purgé intégralement — 0 résidu après (bail/locataire/bien/quittance/bailleur2/gestionnaire tous à 0). **Gate Staging Sprint C GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint-c-ep15-decision.md`. Migration **non additive** (RSV-EP15-03) : rollback applicatif non viable, restauration de backup seule. Production non autorisée ; Gate Production distinct requis avec Préflight renforcé (backup avant/après migration). |
| 2026-07-19 | `sha-e4744d92` | EP-16 Sprint N Fondation notifications (PR #235, régularisation #236) | **9/9** | **63/0** | 5/5 / `BackupHeartbeatMissing` historique | CI et CodeQL post-fusion verts ; images immuables API/Web confirmées. Sauvegarde `loyertracker-staging-20260719-152116.dump` (511266 octets, 799 entrées, SHA-256 `9b99dcf6…`) + globals. STG-ISOL-01 PASS avant/après : NPM et 9 conteneurs intacts, restart=0, seuls `api`/`nginx` recréés. Flyway **V27 appliquée, 27/27** ; RLS ENABLE+FORCE sur `notification_preference/event/outbox/delivery`, référentiel `notification_template` global sans RLS. Smoke **63/0**. Preuve EP-16 : 24 événements (`BAIL_CREE` 1, `PAIEMENT_RECU` 1, `LOYER_EN_RETARD` 21, `PREAVIS` 1), **0 préférence, 0 Outbox, 0 Delivery, 0 log Twilio** — aucun envoi externe sans consentement. **Gate Staging GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint-n-ep16-decision.md`. Production non autorisée ; Gate Production distinct requis. |
| 2026-07-24 | `sha-27dce09d` | EP-16 Sprint N+1 WhatsApp P0 (PR #252 + correctif lien de vérification PR #254, merge `27dce09`) | **9/9** | **63/0 + vérification manuelle WhatsApp réelle** | 5/5 / 0 alerte | CI et CodeQL post-fusion verts (Quality Gate Sonar validé après revue PO d'un Security Hotspot `java:S4790`, HmacSHA1 imposé par Twilio). Sauvegarde `loyertracker-20260724-125743.dump` (541465 octets, SHA-256 `24aeeac3…`) + globals. STG-ISOL-01 PASS avant/après : NPM et 9 conteneurs intacts, restart=0, seuls `api`/`nginx` recréés. Flyway **V28 appliquée, 28/28** (seed 3 templates P0 + 2 fonctions `SECURITY DEFINER`). Smoke **63/0**. **Premières variables Twilio réelles sur cet hôte mutualisé** (compte Sandbox du PO, jamais versionné). **Vérification manuelle du parcours complet en conditions réelles** : quittance émise → Outbox → dispatch automatique (15 s) → WhatsApp livré à `+243999964331` (confirmé côté Twilio : `status: delivered`) → lien de vérification résolu `VALIDE` → confirmation visuelle du PO. `PAIEMENT_RECU` (sans template) → `DEAD` sans envoi, confirmant le critère GO en conditions réelles. **Écart NPM découvert et corrigé après confirmation PO** (voir §5 écart 6) : basic-auth NPM bloquait la page publique `/verify/` ; correctif `location /verify/ { auth_basic off; }` appliqué à `18.conf` + `advanced_config` SQLite persisté, même patron que le bypass `/api/` (écart 5). Nettoyage transactionnel complet, 0 résidu. **Gate Staging Sprint N+1 GO — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint-n1-ep16-decision.md`. Aucune activation Production ; Sandbox Twilio exclusivement (ADR-18, K4) ; Gate Production distinct requis. |
| 2026-07-29 | `sha-ac374193` | EP-16 Sprint N+2 Lot A — fallback SMS + observabilité (PR #291, merge `ac374193`) + correctif transmission env `docker-compose.staging.yml` (PR #300) | **9/9** | **63/0 (après correctif compteur Flyway hôte, cf. note)** | non re-vérifié, sans rapport | Ré-instruction du Gate Staging suite à la confirmation PO d'un numéro Sandbox SMS Twilio. **Prérequis technique découvert et corrigé** : `docker-compose.staging.yml` ne transmettait pas `TWILIO_SMS_FROM`/`NOTIFICATION_FALLBACK_ENABLED`/`NOTIFICATION_BUDGET_MENSUEL_MAX`/`NOTIFICATION_BUDGET_SEUIL_ALERTE` au conteneur `api` (PR #300, mergée avant ce déploiement). Sauvegarde `loyertracker-20260729-164100.dump` (540 Kio) vérifiée. STG-ISOL-01 PASS avant/après (9 conteneurs `loyertracker-staging-*` + `nginx-proxy-manager` intacts, ports/réseaux/volumes inchangés). Déploiement ciblé `api` uniquement (`nginx` reste `sha-27dce09d`, aucun changement Web dans ce lot). **Dérive hôte découverte et corrigée** (hors périmètre applicatif, fichiers d'exploitation seulement) : le fichier `docker-compose.staging.yml` local à l'hôte utilisait encore `LOYERTRACKER_TAG` alors que `main` est passé à `API_IMAGE_REF`/`WEB_IMAGE_REF` (pin digest, RSV-MIG-611-05) — non résolu ici, signalé en réserve ; `infra/smoke/smoke-stack.sh` et `infra/release/production-state.env` étaient absents/obsolètes sur l'hôte (compteur Flyway codé en dur à 28) — synchronisés depuis `main` (mêmes fichiers, aucune divergence fonctionnelle par ailleurs), 1er passage 1 FAIL (attendu) → **63/0 après synchronisation**. Flyway 29/29 inchangé (V29 déjà appliquée depuis Sprint N+2). **Vérification réelle du fallback SMS (US-124)** avec de vrais appels Twilio (pas de mock) : échec WhatsApp synchrone (HTTP 400) → classé `PERMANENT` → fallback SMS réellement déclenché et tenté (`notification_fallback_total{issue="DECLENCHE"}=1`) — mécanisme prouvé de bout en bout. **Limite découverte, non un bug de ce lot** : un échec WhatsApp *asynchrone* (accepté puis rejeté par callback, ex. code Twilio 63015) ne déclenche jamais le fallback (`TwilioCallbackController` ne fait que mettre à jour le statut de livraison) — seul un rejet *synchrone* (4xx immédiat) déclenche `NotificationFallbackService`. Aucun SMS n'a donc atteint un téléphone réel dans ce test ; le mécanisme est validé avec un numéro invalide (rejet synchrone garanti). Données de test synthétiques nettoyées/désapprouvées après usage (template SMS test repassé `BROUILLON`/`enabled=false`). **Gate Staging Sprint N+2 Lot A GO SOUS RÉSERVE — `STAGING_DEPLOYED`**. Décision : `gate-staging-sprint-n2-ep16-decision.md` (addendum 2026-07-29). `RSV-MIG-611-04` (Enterprise Architect) maintenue ouverte, sans effet bloquant. Aucune activation Production ; Gate Production distinct requis. |
| 2026-08-03 | _(aucun changement API/Web — `sha-ac374193`/`sha-27dce09d` inchangés)_ | EP-17 Lot 4 Pilote Keycloak — câblage du thème (`4509049`), audit sécurité `ADR-UI-001` (`f62d646`), locale française `DD-EP17-13` (`d480bb1`, merge `b6b9c7c`) | **9/9** | **63/0 (api/nginx non touchés, non-régression)** | non re-vérifié, sans rapport | Preuve Staging attendue par `DD-EP17-01`. **Dérive hôte résolue au préalable** (hors périmètre applicatif) : dépôt à `27dce09` (~40 commits de retard), 2 fichiers modifiés localement (`docker-compose.staging.yml`, `infra/smoke/smoke-stack.sh`) au contenu déjà identique à `main` (`git checkout --` sans perte) et `infra/release/production-state.env` non suivi, snapshot obsolète de la release `1.14.0` remplacé par la version versionnée `1.15.0` — déplacé (`.superseded-2026-08-03`, pas supprimé) pour débloquer `git pull --ff-only` `27dce09..b6b9c7c`. **Déploiement ciblé, 3 services uniquement** (`keycloak`, `keycloak-init`, `keycloak-theme-init`) — `api`/`nginx`/`postgres`/monitoring non recréés. STG-ISOL-01 PASS avant/après (16:50/16:53 UTC) : 9 conteneurs `loyertracker-staging-*` + `nginx-proxy-manager` identiques, seul `keycloak` recréé (nouveau montage du thème), aucun autre projet affecté. Logs `keycloak-theme-init` : activation `loginTheme` puis locale française confirmées (exit 0). **Vérification réelle de l'écran de connexion** sur le port interne (`https://localhost:18443`), flux OIDC/PKCE réel (`code_challenge`/`code_verifier` générés, client public `loyertracker-spa`, aucun raccourci) : `<html lang="fr">`, titre « Se connecter à LoyerTracker », corps « Connectez-vous à votre compte », classe `card-pf` du thème présente, **0** occurrence `kc-locale` (sélecteur de langue absent comme attendu, une seule locale supportée) — identique à la vérification Dev. Smoke **63/0** (`api`/`nginx` non recréés — confirme l'absence de régression, ne teste pas le thème lui-même, hors périmètre du script). **`DD-EP17-01` close** au registre de dette — preuve Staging complète. **Gate Staging Lot 4 Keycloak GO — `STAGING_DEPLOYED`**. Aucune activation Production ; Gate Production distinct requis (hors périmètre CGPA pour un pilote Keycloak sans Account Console, cf. Plan d'Exécution §3 Lot 4). |
| 2026-08-05 | API `sha256:2522ae210603...`, Web `sha256:9be1a4cd8b...` | EP-18 Canal EMAIL via Resend — Gate Staging **GO** (`dbded12`, PR #370 incluse ; PR #371 doc reserve) | **8/8 + NPM intact** | **63/0** | EMAIL reçu PO / webhook reporté EP-19 | Déploiement technique ciblé `api`+`nginx`, puis `api` seul pour kill-switch sécurité. STG-ISOL-01 PASS avant/après : 9 conteneurs hôte, 8 `loyertracker-staging-*`, réseaux 5, volumes 28, `nginx-proxy-manager` inchangé. Backup pré-déploiement `loyertracker-20260805-161025.dump` vérifié (548K). Flyway **31/31** avec V30/V31 appliquées. Healthchecks UP, Actuator 200, `/healthz` 200. Test invitation EMAIL contrôlé : token retrouvé et valide ; `onboarding@resend.dev` accepté (`QUEUED` + `provider_message_id`) et réception PO confirmée sur `jptshilombo373@gmail.com`. Arbitrage PO : aucun domaine `staging.loyerpro.org` requis pour ce Gate. Réserve restante : aucun webhook réel observé. `RESEND_EMAIL_ENABLED=false` remis après test. Décision : `gate-staging-ep18-notifications-email-resend-decision.md`. Gate Production distinct requis ; `RSV-EP18-06` maintenue. |

> Le Staging applicatif actif depuis le Gate EP-18 est **EP-18 par digest GHCR** :
> API `sha256:2522ae210603...` et Web `sha256:9be1a4cd8b...`. Le Gate est **GO**
> pour l'envoi/réception EMAIL (`onboarding@resend.dev`, réception PO confirmée, Provider Message ID
> `fdac0ef4-a19f-4893-9f89-abe55b1f25c8`) ; le domaine `staging.loyerpro.org` est sans objet ; le webhook réel est reporté EP-19 ; `RESEND_EMAIL_ENABLED=false` maintient le kill-switch
> EMAIL en sécurité. Flyway **31/31**, smoke **63/0**, STG-ISOL-01 PASS avant/après. Aucune activation
> Production. La Production reste soumise à un Gate distinct.
>
> Historique — le **`sha-27dce09d`** (EP-16 Sprint N+1 WhatsApp P0, déployé le 2026-07-24) était
> le tag actif précédent. Flyway **28/28**, smoke **63/0**, STG-ISOL-01 PASS avant/après, parcours
> WhatsApp réel vérifié de bout en bout (Sandbox Twilio).
>
> Historique — le **`sha-e4744d92`** (EP-16 Sprint N Fondation notifications, déployé le
> 2026-07-19) était le tag actif précédent. Flyway **27/27**, smoke **63/0**, STG-ISOL-01 PASS
> avant/après, 24 événements métier et 0 Outbox/Delivery sans consentement.
>
> Historique — le **`sha-359f4d63`** (Sprint C EP-15, déployé le 2026-07-17) était le tag actif
> précédent et a depuis été promu en Production `1.12.0`. Flyway **26/26**, smoke **63/0**,
> STG-ISOL-01 PASS avant/après, backfill Bail→Locataire vérifié.
>
> Historique — le **`sha-cba13f52`** (Sprint unique EP-13 Fin de bail, déployé le 2026-07-16)
> était un tag actif antérieur. Flyway **25/25**, smoke **62/0** + vérification manuelle dédiée **22/0**,
> STG-ISOL-01 PASS avant/après. Clôture/réouverture de bail, purge de l'échéancier futur et
> non-régression des alertes confirmées en conditions réelles. La Production reste soumise à un
> Gate distinct.
>
> Historique — le `sha-c9200a51` (Sprints A+B EP-15, déployé le 2026-07-15) était le tag actif
> précédent, promu en Production `1.10.0` (release clôturée le 2026-07-16). Flyway **24/24**,
> smoke **62/0** + vérification manuelle dédiée **48/0**, STG-ISOL-01 PASS avant/après. Cycle de
> vie Gestionnaire (statut global, suspension/réactivation, archivage cross-tenant conditionné) et
> entité Locataire (RLS standard, CRUD, cycle de vie) confirmés en conditions réelles, y compris
> les deux correctifs du PR #209.
>
> Historique — le `sha-75646d8f` (Sprint 12 EP-14b, déployé le 2026-07-06) était le tag actif
> précédent, promu en Production `1.9.0` (release clôturée le 2026-07-08). Flyway **22/22**, smoke
> **62/0**, STG-ISOL-01 PASS avant/après. Vérification publique réelle `QT-2026-000001` VALIDE, PDF
> intègre, métriques et rate-limit 429 confirmés.
>
> Historique — le `sha-2c5f43c7` (Sprint 10 EP-12b US-95/96/97 + correctif RSV-S10-01, déployé le
> 2026-07-04 à ~15:40 UTC par arbitrage PO S1) était le tag actif précédent, avec exposition
> publique sur `https://loyertracker.staging.loyerpro.org`. Flyway **21/21** (inchangé — le
> correctif ne porte aucune migration). Smoke **59/0** au premier passage. STG-ISOL-01 PASS
> avant/après. Ordre intra-jour du ledger vérifié en direct sur l'API réelle (preuve
> discriminante vs ancien tri UUID). **Ce tag est le candidat `PRODUCTION_READY` de la release
> `1.8.0`.** **Réserves** : RSV-S9-03 (héritée, acceptée permanente) ; OBS-S10-01 (observation
> cosmétique, non bloquante — ordre des lignes backfillées V20 à `cree_le` identique).
>
> Historique — le `sha-1d1c2a5d` (Sprint 10 EP-12b US-95/96/97, déployé le 2026-07-04 à
> ~10:59 UTC) était le tag actif précédent. Flyway **21/21** (V21 ajoutée :
...
> invariant ledger 4/4). RSV-S10-01 levée le 2026-07-04 (tri stable, PR #173 — correctif absent
> de ce tag, embarqué dans `sha-2c5f43c7`).
>
...
**Extension EP-17 Lot 4 (thème Keycloak) — PASS le 2026-08-03** : le nouveau service `keycloak-theme-init`
et le montage du thème (`./infra/keycloak/themes/loyertracker:/opt/keycloak/themes/loyertracker:ro`)
n'introduisent ni port, ni volume nommé, ni réseau externe (bind-mount lecture seule + one-shot sur
le réseau `loyertracker-net` déjà dédié). Confirmé live : 9 conteneurs `loyertracker-staging-*` +
`nginx-proxy-manager` identiques avant/après, seul `keycloak` recréé. Détail complet §8, entrée du
2026-08-03.
