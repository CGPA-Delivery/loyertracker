...
> Le **`sha-ac374193`** (EP-16 Sprint N+2 Lot A, déployé le 2026-07-29) est le
> **tag actif en staging** pour l'API (`nginx` reste `sha-27dce09d`, aucun changement Web dans ce
> lot). Flyway **29/29**, smoke **63/0**, STG-ISOL-01 PASS avant/après, fallback SMS US-124 validé
> avec de vrais appels Twilio (limite : rejets asynchrones non couverts, cf. §8). Aucune activation
