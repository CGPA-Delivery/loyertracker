...
| 2026-08-05 | API `sha256:2522ae210603...`, Web `sha256:9be1a4cd8b...` | EP-18 Canal EMAIL via Resend — Gate Staging **NO GO** (`dbded12`, PR #370 incluse) | **8/8 + NPM intact** | **63/0** | non validé : Resend `HTTP 401` | Déploiement technique ciblé `api`+`nginx`, puis `api` seul pour kill-switch sécurité. STG-ISOL-01 PASS avant/après : 9 conteneurs hôte, 8 `loyertracker-staging-*`, réseaux 5, volumes 28, `nginx-proxy-manager` inchangé. Backup pré-déploiement `loyertracker-20260805-161025.dump` vérifié (548K). Flyway **31/31** avec V30/V31 appliquées. Healthchecks UP, Actuator 200, `/healthz` 200. Test invitation EMAIL contrôlé : token retrouvé et valide ; domaine Staging attendu refusé `HTTP 403` (`RESEND_REFUS_403`, domaine non vérifié) ; domaine test Resend accepté (`QUEUED` + `provider_message_id`) et réception PO confirmée sur `jptshilombo373@gmail.com`, mais aucun webhook réel observé. `RESEND_EMAIL_ENABLED=false` remis après test. Décision : `gate-staging-ep18-notifications-email-resend-decision.md`. Production EP-18 interdite ; `RSV-EP18-06` maintenue. |

> Le Staging applicatif actif depuis le Gate EP-18 est **EP-18 par digest GHCR** :
> API `sha256:2522ae210603...` et Web `sha256:9be1a4cd8b...`. Le Gate est **NO GO**
> fonctionnel côté domaine Staging Resend (`HTTP 403`, domaine non vérifié) et webhook réel ; réception via domaine test Resend confirmée par PO ; `RESEND_EMAIL_ENABLED=false` maintient le kill-switch
