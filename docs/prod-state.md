...
>
> **Validation finale PASS le 2026-08-05 ~19:25 UTC** (`validation-finale-ep18-notifications-email-resend-report.md`) : smoke Production canonique **65 PASS / 0 FAIL** (RUN_ID `1785957904`), JWT réels Keycloak via Nginx TLS, invitation → acceptation Admin API, isolation cross-tenant, RGPD, alertes/audit et garde-fous ports validés. Nettoyage transactionnel complet : 47 lignes DB supprimées, 2 comptes Keycloak smoke supprimés, résidus RUN_ID `0`, `directAccessGrants=false`, `bailleur-test=false`.
>
> **Resend reste désactivé en Production** : `RESEND_EMAIL_ENABLED=false`, `RESEND_FROM_EMAIL` absent, `notification_outbox`/`notification_delivery` final à `0/0`; aucun envoi externe activé. **`PRODUCTION_DEPLOYED` confirmé pour `1.16.0`**. Hypercare **T0 PASS** le 2026-08-05T19:57:29Z : stack healthy, Flyway 31/31, baseline inchangée, `directAccessGrants=false`, `bailleur-test=false`; alertes `NotificationKillSwitchFerme` et `BackupHeartbeatMissing` qualifiées non bloquantes. Pré-check T+12 anticipé le 2026-08-05T21:05:52Z : **PASS technique / WARN fenêtre**, aucun critère de suspension Production atteint, Resend toujours fermé, `outbox`/`delivery=0/0`, `BackupHeartbeatMissing` non bloquante car cron `02:15 UTC` non encore rejoué. Pré-check T+24 anticipé le 2026-08-05T21:57:20Z : **PASS technique / WARN fenêtre**, aucun critère de suspension Production atteint, Resend toujours fermé, `outbox`/`delivery=0/0`, alertes requalifiées non bloquantes au moment du contrôle. **Décision PO 2026-08-05T22:10Z : valeur probante anticipée T+12/T+24 acceptée ; hypercare EP-18 terminé sans incident bloquant.** Rapports : `docs/cgpa/09-production/deploiement-technique-ep18-notifications-email-resend-report.md`, `docs/cgpa/09-production/validation-finale-ep18-notifications-email-resend-report.md`, `docs/cgpa/09-production/plan-etape-hypercare-v1.16.0.md`.
>
> **Activation Resend Production EP-18 — `RESEND_PRODUCTION_ENABLED` (2026-08-06T00:21:46Z)** : sur instruction PO explicite, `RESEND_EMAIL_ENABLED=true`, `RESEND_FROM_EMAIL=onboarding@resend.dev`, `NOTIFICATIONS_EXTERNAL_ENABLED=true`, `NOTIFICATION_DRY_RUN=false`, budget conservateur `NOTIFICATION_BUDGET_MENSUEL_MAX=100`, seuil `0.8`. Correctif opérationnel associé : propagation Compose de `NOTIFICATION_BUDGET_MENSUEL_MAX` et `NOTIFICATION_BUDGET_SEUIL_ALERTE`, sinon l'API restait bloquée à `0/0`. Recréation ciblée `api` uniquement ; Nginx/PostgreSQL/Keycloak inchangés ; `/healthz` et racine publique `200`, release lock cohérent, outbox/delivery `0/0`, budget métrique `0/100`. Rapport : `docs/cgpa/09-production/activation-resend-production-ep18-report.md`.
...
> (`bailleur-test` déjà `enabled=false`, contrairement à `1.13.0`). **Volet notifications validé en
> conditions réelles** : le run a généré 8 `notification_event` (5 `LOYER_EN_RETARD`,
> 1 `PAIEMENT_RECU`, 1 `BAIL_CREE`, 1 `PREAVIS`) tandis que `notification_outbox` et
> `notification_delivery` sont restés à **0** — condition 6 du Gate Production satisfaite, aucun
> envoi externe possible. Nettoyage transactionnel de **47 lignes** en une seule transaction (dont
> les 8 `notification_event` du run, les 17 préexistants appartenant à un bailleur réel), 2 comptes
> Keycloak supprimés, `bailleur-test` redésactivé. Baseline métier et tables `notification_*`
> restaurées à l'identique. **`PRODUCTION_DEPLOYED` prononcé rétroactivement à la date de sa
> preuve** (2026-07-27), le déploiement technique réel datant du 2026-07-24 — même principe que la
> régularisation `1.11.0`. Détail : `validation-finale-v1.14.0-report.md`. **Restent à instruire :
> hypercare T0/T+12/T+24 puis clôture CDO.** L'écart de gouvernance `R-V54-2` reste **ouvert**.

### Constat initial du 2026-07-27 (avant validation finale)
...
| Smoke | Au constat initial : **NON EXÉCUTÉ**. **Exécuté le 2026-07-27 : 63 PASS / 0 FAIL au premier passage** (RUN_ID `1785170429`) |
| Services | 8/8 actifs, 4/4 healthy, `RestartCount=0` sur `api`/`nginx` |
| Observabilité | Prometheus 5/5 `up` ; Alertmanager **0 alerte** ; 0 ligne 5xx ; `/healthz` et site public 200 |
| Baseline post-validation | 3 bailleurs, 2 patrimoines, 8 biens, 8 baux, 8 garanties, 1 gestionnaire, 8 locataires, 7 quittances — identique à l'état pré-test |
| Tables `notification_*` post-validation | 17 `event`, 0 `outbox`, 0 `delivery`, 3 `template`, 0 `preference` — identiques à l'état pré-test |
| État CGPA | **`PRODUCTION_DEPLOYED` atteint le 2026-07-27 ~16:46 UTC** (déploiement technique réel du 2026-07-24, statut prononcé à la date de sa preuve) |
...
`docs/cgpa/09-production/validation-finale-v1.14.0-report.md`.
...
acceptée par le PO — permanente pour ce schéma).
**RP-160-03 levée le 2026-07-03** (`CHANGELOG.md` scindé en `[1.5.0]`/`[1.6.0]`/`[1.7.0]`).
**RSV-S7-8-01 levée le 2026-07-03** (vérification visuelle USD réelle + CDF synthétique sur
`ai-test-server`, nettoyée — détail `docs/project-state.md`).
