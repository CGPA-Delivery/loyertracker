...
>
> **Resend reste désactivé en Production** : `RESEND_EMAIL_ENABLED=false`, `RESEND_FROM_EMAIL` absent, `notification_outbox`/`notification_delivery` final à `0/0`; aucun envoi externe activé. **`PRODUCTION_DEPLOYED` confirmé pour `1.16.0`**. Prochaine étape distincte : hypercare T0/T+12/T+24, puis clôture CDO. Rapports : `docs/cgpa/09-production/deploiement-technique-ep18-notifications-email-resend-report.md`, `docs/cgpa/09-production/validation-finale-ep18-notifications-email-resend-report.md`.
