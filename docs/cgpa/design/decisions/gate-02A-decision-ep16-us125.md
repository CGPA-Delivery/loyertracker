...
| DDS-cand-1 | Réserve non bloquante | Emplacement préférences côté Gestionnaire non tranché | Product Owner | Design Architect | Avant Gate 04A | DDS formalisée dans `design-decision-register.md` | **Levée le 2026-07-31** — `DDS-LT-002` Acceptée, validation Product Owner obtenue |
| DDS-cand-2 | Réserve non bloquante | Filtre/pagination historique non tranché | Product Owner | Design Architect | Avant Gate 04A | DDS formalisée | **Levée le 2026-07-31** — `DDS-LT-003` Acceptée, validation Product Owner obtenue |
| DDS-cand-3 | Réserve non bloquante | Mapping statuts Outbox/Delivery non tranché formellement | Product Owner | Design Architect | Avant Gate 04A | DDS formalisée | **Levée le 2026-07-31** — `DDS-LT-004` Acceptée, validation Product Owner obtenue |
| DDS-cand-4 | Réserve non bloquante | Premier modal du produit, sans précédent focus-trap/restitution focus | Product Owner | Design Architect | Avant Gate 04A | DDS formalisée + `CHECK-ACCESSIBILITY-01` dédié | **Levée le 2026-07-31** — `DDS-LT-005` Acceptée ; `CHECK-ACCESSIBILITY-01` reste à exécuter avant tout GO Gate 04A |
