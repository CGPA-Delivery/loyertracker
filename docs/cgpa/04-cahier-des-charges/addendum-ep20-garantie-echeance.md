...
| EF-149 | Dans la même transaction : solde garantie, `RETENUE_LOYER`, paiement lié, audit riche et événement/Outbox sont cohérents; aucune opération réseau. La décision produit détermine si la cardinalité reste « un mouvement par paiement » (contrainte DB explicite) ou devient « plusieurs mouvements » (modèle de liaison append-only). |
