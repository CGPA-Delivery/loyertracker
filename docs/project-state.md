> **Lot 1 / DD-EP17-14 — SMTP Keycloak : Staging GO, préflight Production NO GO temporaire (2026-08-12).** La configuration SMTP **Amazon SES Mail Manager** (`587` + STARTTLS) est validée en Staging (PR #456, parcours PO/CDO et anti-énumération PASS : `HTTP 200` + message identique). Le préflight Production a confirmé API/Web/Keycloak/PostgreSQL healthy, observabilité active, lock release cohérent et backup quotidien présent ; mais le SMTP Keycloak et son mécanisme de déploiement sont **absents en Production**, et le checkout hôte est antérieur/dirty. Aucun déploiement Production autorisé. Critères : mécanisme dédié/idempotent, config candidate immutable, backup hashé, rollback `smtpServer`, `CHECK-REL-01`, `CHECK-OPS-01`, tests contrôlés et décision CDO explicite. Référence : `docs/cgpa/09-production/gate-production-dd-ep17-14-preflight.md`.
>
