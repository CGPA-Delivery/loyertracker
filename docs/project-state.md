> **Lot 0 — clôture documentaire de 4 points (2026-08-12).** `DD-EP17-10` (état d'erreur chargement listes) close : l'équivalence ARIA `role="alert"` de `lt-data-table` est acceptée par le CDO, sémantique plus forte que `role="status"`, aucun changement applicatif requis. `DD-EP17-11` (touch targets button < 44px) close : le code source `_button.scss` contient déjà `min-height: 44px`, le token `--lt-control-height-lg: 44px` est défini, les tests unitaires confirment `getComputedStyle(...).minHeight === '44px'`. `RSV-S9-03` reclassée de « réserve active » → « contrainte permanente acceptée » (décision PO 2026-07-03, migration V20 destructive, rollback = restore DB complet). `OBS-S10-01` déjà CLOSE par arbitrage PO du 2026-07-05 (`arbitrage-obs-s10-01.md`) — synchronisation documentaire. Plan d'exécution complet pour les 12 points restants : `docs/cgpa/design/decisions/plan-execution-consolide-2026-08-12.md`.
>
