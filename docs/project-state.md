> **EP-20 — Paiement d’une échéance par garantie locative : cadrage intégré / NO GO applicatif (2026-08-13).** PR #479 fusionnée dans `main` (`d0ca222`). Audit sur le codebase : le flux US-95 existe (ledger `RETENUE_LOYER`, FK V21, audit, Outbox), mais une retenue après paiement classique remplace `montant_recu` et compare la retenue isolée au montant attendu ; le cas 400 + 600 sur 1 000 produit donc à tort `PARTIEL`. Quittance ADR-15, lien HMAC, Outbox/consentement/fallback/callback existent; restent à compléter le calcul cumulatif et la concurrence/cardinalité, le payload/rendu `GARANTIE_DEBITEE` avec `bien_id` persistant, les templates SMS, le document `PARTIEL`, la nomenclature PDF commune et l’UI de confirmation/résultat. EP-20/US-148→153 sont proposés sans collision (EP-19 préservé). **NO GO applicatif** jusqu’à approbation PO/CDO du Plan et des arbitrages reçu partiel distinct, cardinalité et notification; aucun code, migration, Staging ou Production autorisé. Réserves : `RSV-EP20-PARTIEL-01`, `RSV-EP20-CARDINALITE-02`, `RSV-EP20-NOTIF-03`. Références : `docs/cgpa/06-planification-agile/plan-execution-ep20-garantie-echeance.md`, `rapport-cadrage-ep20-garantie-echeance-2026-08-13.md`.
