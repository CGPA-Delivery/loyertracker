> **Lot 1 / DD-EP17-14 — SMTP Keycloak Staging : GO (2026-08-12).** La configuration SMTP **Amazon SES Mail Manager** (`587` + STARTTLS) est appliquée de façon ciblée au realm Keycloak Staging par PR #456 ; `keycloak-smtp-init` a terminé avec succès et la configuration runtime (hôte, port, expéditeur, auth, STARTTLS) est confirmée sans exposer le secret. Jordan Tshilombo (PO/CDO) confirme que le parcours réel « Mot de passe oublié » sur Staging est **OK**. La preuve anti-énumération est également **PASS** : compte existant et adresse inexistante renvoient tous deux `HTTP 200` et le même message utilisateur après normalisation des sessions dynamiques. **Prochaine étape : Gate Production distinct** (RC/configuration figée, `CHECK-REL-01`, `CHECK-OPS-01`, backup/rollback/observabilité, décision CDO explicite). Production inchangée. Décision : `docs/cgpa/07-devsecops/gate-staging-smtp-keycloak-dd-ep17-14-decision.md`.
>
