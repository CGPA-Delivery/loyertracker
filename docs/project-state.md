> **Lot 1 / DD-EP17-14 — SMTP Keycloak Staging : GO sous réserve de sécurité (2026-08-12).** La configuration SMTP **Amazon SES Mail Manager** (`587` + STARTTLS) est appliquée de façon ciblée au realm Keycloak Staging par PR #456 ; `keycloak-smtp-init` a terminé avec succès et la configuration runtime (hôte, port, expéditeur, auth, STARTTLS) est confirmée sans exposer le secret. Jordan Tshilombo (PO/CDO) confirme que le parcours réel « Mot de passe oublié » sur Staging est **OK**. **Réserve avant Gate Production :** exécuter et consigner le comparatif compte existant / inexistant (même HTTP + même message) pour fermer formellement le canal d'énumération observé avant correctif. Aucun changement Production autorisé. Décision : `docs/cgpa/07-devsecops/gate-staging-smtp-keycloak-dd-ep17-14-decision.md`.
>
