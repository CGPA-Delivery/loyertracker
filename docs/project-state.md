> **EP-17 Lot 5 — Uniformisation visuelle (5 dettes Design, 2026-08-13).** Branche `fix/lot5-uniformisation-visuelle`. **DD-EP17-04** : `AlertesListeComponent`, `AuditJournalComponent` et `GestionnaireDashboardComponent` migrés vers `lt-data-table` (étendu avec colonne `type: 'actions'`). **DD-EP17-06** : 88 occurrences de spacing hardcodé remplacées par tokens `--lt-space-*` dans 15 fichiers. **DD-EP17-07** : convention `data-testid` documentée, 5 composants instrumentés. **DD-EP17-09** : breakpoint `VerifyReceiptComponent` harmonisé (`560px→640px`), `border-radius` ad hoc remplacés par `var(--lt-radius-default)`. **DD-EP17-11** : clôture documentaire — `min-height: 44px` déjà présent dans `_button.scss`. 235/235 tests SUCCESS.
>
