> **EP-19 — Cadrage validé : GO / EP19_CADRAGE_READY (2026-08-21).** Jordan Tshilombo Kabamba, PO/CDO, a validé le cadrage EP-19 (délivrabilité email, Webhooks Resend/Svix) avec périmètre 3 US (6 jours) : US-144 (validation signatures Svix), US-145 (statuts de délivrabilité), US-146 (métriques, alertes + dashboard). **Phase 1 (Backend US-144+US-145) est autorisée.** Conditions : Gate distinct requis pour Production ; webhook Resend/Svix à configurer avant Staging ; secret `whsec_...` stocké hors Git uniquement ; aucun envoi réel supplémentaire autorisé. Référence : `docs/cgpa/06-planification-agile/cadrage-ep19-delivrabilite-email-2026-08-21.md`.

> **EP-17 — Clôture prononcée : GO sous réserve documentaire (2026-08-21).** Après livraison des Lots 1-5 et acceptation explicite des réserves résiduelles (`RES-VR-04`, `RES-BASELINE-01`, `RES-DESIGN-01`, dettes design), EP-17 est **Clôturé — intégré sur `main`**. Décision PO/CDO : `docs/cgpa/06-planification-agile/cloture-ep17-go-reserve-documentaire-2026-08-21.md`. Les réserves sont acceptées comme limitations connues documentées. Aucun code, migration, Staging, Production, provider, secret ou envoi réel n'est autorisé par cette clôture.

> **Synchronisation Product Backlog post-EP-20 autorisée (2026-08-15).** Le PO/CDO autorise une migration documentaire additive : la table historique du 2026-08-10 est préservée et une vue canonique actuelle est ajoutée. EP-17 devient **Partiellement livré / exécuté par lots — clôture globale non prononcée**, avec `EP-17/US-142` non approuvée ; EP-19 reste **Proposé — non approuvé**, la fondation EP-18/US-143 ne valant pas livraison EP-19 ; EP-20 est **Clôturé — intégré sur `main`** après PR #501 (`293dd50`). Aucun EP-21, code, migration, Staging, Production, provider, secret, webhook ou envoi réel n’est autorisé. Référence : `docs/cgpa/06-planification-agile/decision-pocdo-synchronisation-backlog-post-ep20-2026-08-15.md`.
>
> **Résolution alerte Dependabot `js-yaml` HIGH — PR #503 fusionnée (2026-08-15).** La vulnérabilité `GHSA-5p4m-2wfm-xmqj` (`js-yaml` 4.3.0) est corrigée par la montée en 4.3.1. La PR #503 a été fusionnée dans `main` (`ef1e97d`) après CI complète SUCCESS (Backend, Frontend, sécurité, CodeQL, Build/scan/SBOM Docker). Validation locale : `npm ci`, garde sécurité, 241/241 tests Angular, lint et build production PASS. **Alertes Dependabot restantes sur `main` : 3 Medium (`hono` 2×, `postcss` 1×) et 1 Low (`hono` 1×)**, toutes en scope `development` (toolchain de build), sans exposition runtime dans l’image Nginx. Aucune HIGH ouverte. Aucun Staging, Production, provider, secret ou envoi réel. Les alertes Medium/Low restantes seront traitées par PR séparées sans `npm audit fix --force`.
>
> **Résolution alerte Dependabot `hono` — PR #506 fusionnée (2026-08-20).** Trois alertes Dependabot (`#32` Medium / `#34` Medium / `#33` Low) sur `hono` (`GHSA-54fx-42gc-7vw4`, `GHSA-f23p-vx2j-j53r`, `GHSA-79qm-7rj5-m7r9`) sont corrigées par la montée globale en 4.13.2. La PR #506 a été fusionnée après CI complète SUCCESS. Aucune exposition runtime (dépendance de build). Aucun Staging, Production, provider, secret ou envoi réel.
>
> **Résolution alerte Dependabot `postcss` — PR #508 fusionnée (2026-08-20).** Une alerte Dependabot (`#30` Medium) sur `postcss` (`GHSA-fxqj-rqcc-2cmp`) est corrigée par la montée globale en 8.5.25. La PR #508 a été fusionnée après CI complète SUCCESS. Aucune exposition runtime (dépendance de build). Aucun Staging, Production, provider, secret ou envoi réel.
>
> **Résolution alerte Dependabot `fast-uri` — PR #510 fusionnée (2026-08-20).** Une alerte Dependabot (`#37` High) sur `fast-uri` (`GHSA-7p8r-x3mc-p8w7`) est corrigée par la montée globale en 3.1.5. La PR #510 a été fusionnée
>
> **Réserves DevSecOps (2026-08-20).** Les réserves suivantes demeurent actives et nécessitent une action humaine ou un suivi :
> 1. **Angle mort Secret Scanning dépôt** : détecté comme **désactivé** sur le dépôt `CGPA-Delivery/loyertracker`. Nécessite activation par un compte admin (Settings > Code security > Secret scanning). Requis par CGPA v6.1 / Gitleaks.
> 2. **Secret `GITLEAKS_LICENSE`** : non transmis aux PR Dependabot (limite GitHub Actions : secrets non montées sur PR Dependabot). Les branches Dependabot échouent CI sur `gitleaks` mais peuvent être reprises par branche dédiée (pattern éprouvé avec #510).
> 3. **Résidus `npm audit`** : 4 vulnérabilités résiduelles dev-only détectées par `npm audit` mais non remontées comme alertes Dependabot ouvertes (`socket.io-parser`, `less`, `image-size`, `uuid`). Correctifs disponibles en version majeure, non bloquants pour Gate Staging.
>
> **EP-20 — EP20-US06 clôturée, intégrée sur `main` (2026-08-15).** Après les merges #491 à #500 et l’approbation PO/CDO, la story Must est **CLOSE**. La retenue exige une confirmation explicite ; `PARTIEL` respecte ADR-15 sans quittance certifiée ; Escape, focus initial/restitution et confinement Tab/Shift+Tab sont prouvés. Le dashboard Gestionnaire est accessible sans affaiblir OIDC, ReBAC/RLS, Audit ni l’intercepteur. Backend `266` tests sur `35` migrations dépôt ; Frontend `241/241`, lint/build, dialogue `4/4` et matrice responsive authentifiée `24/24` aux viewports `360/390/640/1024` ; PR #500 CI complète et Build/scan/SBOM Docker SUCCESS. Production réelle inchangée à Flyway `32`; aucun Staging/Production, provider, secret ou envoi réel. Référence : `docs/cgpa/06-planification-agile/cloture-ep20-us06-confirmation-responsive-2026-08-15.md`. EP-20 est clôturé jusqu’à nouvelle instruction explicite.
>
> **EP-20 — EP20-US06 : preuve locale responsive finale candidate à revue (2026-08-15).** Sur la branche dédiée `feat/ep20-us06-garantie-responsive-proof`, le seed contrôlé via API métier retourne `15 PASS / 0 FAIL` et restaure `directAccessGrantsEnabled=false`. Le scénario authentifié du dialogue de retenue est GREEN à `360/390/640/1024` : `PARTIEL` conforme ADR-15, zéro POST avant confirmation/Escape, focus initial et restitution, confinement Tab/Shift+Tab, cibles `44 px`, Axe sans violation bloquante, absence d’overflow et captures locales. Suite Angular `241/241`, lint, build et matrice complète `24/24` sont GREEN. Aucun changement OIDC, ReBAC/RLS, Audit, intercepteur, API ou migration ; aucun Staging/Production. Référence : `docs/cgpa/evidence/ep20-us06/local-responsive-dialog-retention-proof-2026-08-15.md`. PR, CI et fusion humaine restent requises avant clôture.
>
> **EP-20 — EP20-US06 : addendum audit Gestionnaire/ReBAC prêt pour revue (2026-08-15).** Le diagnostic runtime identifie `GET /api/audit` (Bailleur-only) comme refus `403` du dashboard Gestionnaire ; la route OIDC et la lecture des biens ne sont pas en défaut. L’addendum `addendum-pocdo-ep20-us06-gestionnaire-audit-rebac-2026-08-15.md` borne l’instruction documentaire au scoping fail-closed par bien affecté et conditionne toute provenance durable/migration à une décision séparée. Aucun code, API, RLS, migration, Staging, Production, provider, secret ou envoi réel n’est autorisé.
>
> **EP-20 — EP20-US06 : addendum lecture Gestionnaire/ReBAC prêt pour revue (2026-08-15).** L’exécution responsive locale a prouvé que `/gestionnaire` est servi mais que le chargement des biens affectés reçoit `403`; l’intercepteur Frontend préserve le refus et ne doit pas être contourné. L’addendum `addendum-pocdo-ep20-us06-gestionnaire-rebac-2026-08-15.md` autorise uniquement l’instruction documentaire du contrat Backend/ReBAC/RLS, des preuves affecté `200` / non affecté et cross-tenant `403`, et de la nécessité éventuelle d’une migration. Aucun code, API, RLS, migration, Staging, Production, provider, secret ou envoi réel n’est autorisé avant fusion et décision PO/CDO distincte.
>
> **EP-20 — EP20-US06 : GO Frontend borné (2026-08-14).** Le PO/CDO autorise, après fusion #491/CI SUCCESS, une future branche Frontend dédiée à la confirmation et restitution UI. Contrats serveur existants seulement; tout manque Backend/API/migration stoppe l’exécution et requiert un addendum distinct. TDD, E2E réels, a11y et responsive `360/390/640/1024` sont obligatoires. Aucun Staging, Production, provider, secret ou envoi réel. Référence : `docs/cgpa/06-planification-agile/decision-pocdo-ep20-us06-go-frontend-2026-08-14.md`.

> **EP-20 — EP20-US06 : GO cadrage documentaire (2026-08-14).** Le PO/CDO autorise exclusivement la conception de la confirmation et restitution UI : contrats, critères GWT, matrice responsive `360/390/640/1024`, a11y et inventaire composants. Aucune preuve navigateur n’est encore déclarée. Aucun code Frontend/Backend, API, migration, Staging, Production, provider, secret ou envoi réel n’est autorisé. Référence : `docs/cgpa/06-planification-agile/decision-pocdo-ep20-us06-ui-cadrage-2026-08-14.md`; l’implémentation reste soumise à une décision PO/CDO post-fusion distincte.

> **EP-20 — EP20-US05 clôturée, intégrée sur `main` (2026-08-14).** PR #489 fusionnée (`d911044`) après CI complète SUCCESS. Les téléchargements authentifié/public servent `quittance-certifiee-YYYY-MM.pdf`, uniquement dérivé de la période certifiée; la factory refuse tout format hors `YYYY-MM`, sans PII. HMAC, `pdf_hash` / `content_hash` et `404` public indifférencié sont préservés. Tests ciblés et `mvn -q verify` PASS sur Flyway 34; Build/scan/SBOM Docker SUCCESS; publication/signatures `SKIPPED` attendue après classifieur vert. Aucun provider, secret, envoi réel, Staging ou Production. Référence : `docs/cgpa/06-planification-agile/cloture-ep20-us05-nomenclature-pdf-2026-08-14.md`. Toute suite EP20 exige une instruction PO/CDO distincte.

> **EP-20 — EP20-US04 clôturée, intégrée sur `main` (2026-08-14).** PR #487 fusionnée (`d3c9b6d`) après CI complète SUCCESS. ADR-15 est démontré : `PARTIEL` retourne `409` et persiste `0` quittance; une retenue passant à `RECU` produit une quittance `EMISE` unique seulement si le profil documentaire est complet. L’adresse bailleur absente préserve impérativement l’écriture financière et laisse la quittance réémissible après correction. `mvn -q verify` PASS sur Flyway 34; publication/signatures `SKIPPED` attendue après classifieur sans image. Aucun provider, secret, envoi réel, Staging ou Production. Référence : `docs/cgpa/06-planification-agile/cloture-ep20-us04-quittance-certifiee-2026-08-14.md`. Toute suite EP20 exige une instruction PO/CDO distincte.

> **EP-20 — EP20-US03 clôturée, intégrée sur `main` (2026-08-14).** Après les merges #484 (`5c8c2c8`, `bien_id` transactionnel/ReBAC fail-closed) et #485 (`b76b3dc`, garde-fou de template SMS), la story Must est **CLOSE**. La preuve TDD couvre le RED d’un fallback créé sans template, puis `0` SMS sans template et `1` SMS unique avec `GARANTIE_DEBITEE/SMS/fr` approuvé/actif. `NotificationDispatchIntegrationTest`, `mvn -q verify` et la CI complète #485 sont SUCCESS sur 34 migrations. V34 est additive; le contrat dépôt est à 34, sans modification du compteur Production. Aucun provider externe, secret, envoi réel, Staging ou Production n’a été activé/exécuté. Référence : `docs/cgpa/06-planification-agile/cloture-ep20-us03-notification-garantie-2026-08-14.md`. **À la clôture US03, EP20-US04 restait non démarrée** et requérait un GO PO/CDO distinct, avec ADR-15 strict (`RECU` seulement; jamais de document certifié pour `PARTIEL`).

> **EP-20 — EP20-US03 : complément SMS candidat à revue (2026-08-14).** Après fusion de la PR #484 (ReBAC `bien_id`), l’audit de clôture a confirmé l’absence de template SMS `GARANTIE_DEBITEE` et un fallback qui mettait un SMS en file sans vérifier son template. Cycle TDD : RED observé (`1` SMS sans template), puis garde-fou fail-closed exigeant code/canal/langue, statut `APPROUVE` et `enabled=true`; migration additive V34 seedant `GARANTIE_DEBITEE/SMS/fr`; tests négatif (`0`) et positif (`1` unique) verts, classe `NotificationDispatchIntegrationTest` complète et `mvn -q verify` PASS sur 34 migrations. Le kill-switch externe reste fermé; aucune activation fournisseur, aucun secret, aucun envoi réel, aucun Staging/Production. Branche `feat/ep20-us03-sms-template-guard`; PR, CI et fusion humaine requises avant clôture formelle US03.

> **EP-20 — EP20-US01 : correctif cumulatif candidat à revue (2026-08-14).** Sur la branche dédiée `feat/ep20-us01-cumul-garantie`, le commit `f5727f2` corrige l’écrasement de `montant_recu` lors d’une retenue de garantie : le montant après opération est désormais cumulé avant dérivation de `RECU`/`PARTIEL`. TDD prouvé : test RED observé (`300 + 550` persistait `PARTIEL`/`550`), GREEN avec `RECU`/`850`, `S03PaiementsGarantiesIntegrationTest` et `mvn -q verify` PASS sur Testcontainers/Flyway 32. PR/revue humaine/CI requises; aucun Staging, Production, provider ou migration.

> **EP-20 — Paiement d’une échéance par garantie locative : décision PO/CDO approuvée / GO borné (2026-08-13).** Après fusion du cadrage PR #479 et de sa synchronisation PR #480, le PO/CDO a statué : `PARTIEL` ne produit aucun document certifié; une seule retenue de garantie est autorisée par échéance; `notification_event.bien_id` est requis avec templates WhatsApp/SMS approuvés avant fallback. Une collision d’IDs avec le Lot 6 UX historique proposé (`US-148→151`) est résolue sans réécriture : EP-20 utilise `EP20-US01→EP20-US06`. **GO / EP20_IMPLEMENTATION_READY** est limité au code TDD, migrations additives strictement nécessaires et tests/CI, et ne devient effectif qu’après fusion humaine de la décision documentaire. Staging, Production, activation Twilio, secret ou envoi réel restent interdits. Référence : `docs/cgpa/06-planification-agile/decision-pocdo-ep20-garantie-2026-08-13.md`.

> **DD-EP17-14 — validation live SMTP Production canonique : T0 PASS / hypercare ouverte (2026-08-13).** Le flux public de réinitialisation a prouvé l'anti-énumération, la remise SMTP, la réception réelle et la consommation de l'action-token. Lock release cohérent, Flyway `32`, cinq services healthy sans restart, HTTPS `200` et Actuator `UP`. La réserve critique `BackupHeartbeatMissing` a été levée par le correctif versionné PR #477 (cron sans interprétation Bash de `.env`), CI SUCCESS, backup contrôlé validé, heartbeat scrapé par Prometheus et aucune alerte `BackupHeartbeat*` active. Aucune mutation SMTP/realm, image, Compose, base ou service pendant le Gate. Hypercare T+12h/T+24h obligatoire ; rapport : `docs/cgpa/09-production/dd-ep17-14-smtp-production-live-validation-t0-2026-08-13.md`.
>
> **DD-EP17-14 — réinstruction contrôlée Gate SMTP Production canonique : GO technique borné (2026-08-13).** Sur l'hôte canonique `18.158.70.88`, `main` et runtime sont alignés, le verrou release est cohérent, le relais SMTP est healthy et `smtpServer` est déjà configuré vers ce relais interne. La réinstruction interdit donc le one-shot de reconfiguration : le prochain Gate ne couvre que la validation live du flux existant, après backup neuf, fenêtre UTC et `GO / PRODUCTION_READY` explicite limité au test. Preuves encore obligatoires : compte actif, réception réelle, action-token, anti-énumération existant/inexistant, santé/logs/alertes et hypercare. Référence : `docs/cgpa/09-production/gate-production-dd-ep17-14-canonical-reinstruction-2026-08-13.md`.
>
> **Production canonique — réconciliation du verrou release : candidat à CI/revue (2026-08-13).** L'autorité CDO confirme que la Production canonique `18.158.70.88` reste sur les artefacts immuables `sha-d19c4fea`. Une dérive documentaire vers les artefacts EP-17 Lot 5 — relatifs à l'hôte legacy non public — est corrigée par une PR dédiée, sans recréation de conteneur, sans changement de realm, de données ni SMTP. Le contrôle public/SSH constate API, Nginx, Keycloak, PostgreSQL et relais SMTP healthy, HTTPS `200`, Actuator `UP`, issuer OIDC canonique et Flyway `32`. Référence : `docs/cgpa/09-production/production-canonical-release-lock-reconciliation-2026-08-13.md`. Toute activation SMTP reste soumise à son Gate séparé.
>
> **EP-17 Lot 5 — Remédiation de dérive Production : PASS (2026-08-13).** Après autorisation PO/CDO, l'hôte Production a été réaligné sur `main` (`c3bb093`), et seules les images API/Web ont été promues par digest CI du commit `f95b68b` — sans rebuild. Backup PostgreSQL + globals + checksum générés hors Git ; PostgreSQL/Keycloak non recréés. API/Nginx/Keycloak/PostgreSQL healthy, HTTPS public `200`, smoke canonique Production **65 PASS / 0 FAIL**. Rapport : `docs/cgpa/09-production/production-ep17-lot5-drift-remediation-report.md`. La synchronisation versionnée de `production-state.env` est soumise par cette PR ; aucun SMTP Keycloak n'a été activé.
>
> **EP-17 Lot 5 — Gate Staging : GO / STAGING_DEPLOYED (2026-08-13).** Les artefacts immuables API `sha256:a7fec330…` et Web `sha256:e1f58934…` issus du merge PR #470 (`f95b68b`) ont été promus sur `ai-test-server`, sans rebuild. `STG-ISOL-01` : seuls `api` et `nginx` recréés ; PostgreSQL, Keycloak et monitoring préservés ; backup pré-déploiement hashé. API/Nginx healthy, API interne `{"status":"UP"}`, HTTPS SPA `200`, smoke réel **63 PASS / 0 FAIL**. Décision : `docs/cgpa/07-devsecops/gate-staging-ep17-lot5-uniformisation-decision.md`. Ce GO est limité à Staging et n'autorise pas Production.
>
> **EP-17 Lot 5 — Uniformisation visuelle (2026-08-13).** PR #470 : `lt-data-table` adopté sur trois composants, tokens de spacing adoptés, convention `data-testid` documentée et `VerifyReceiptComponent` harmonisé. 235/235 tests SUCCESS.
>
> **EP-17 Lot 4 — DMARC progressif + Gate Production `noreply@loyertracker.org` (2026-08-13).** Branche `docs/lot4-dmarc-noreply-gate-prod`. **RSV-DMARC-02** : premier rapport DMARC réel reçu de Google (période 2026-08-12, DKIM=pass, SPF=pass, 0 échec). Script d'analyse `infra/scripts/analyze-dmarc-reports.py` créé et exécuté — 2 rapports, 1 email, 100% DKIM/SPF pass. Cron hebdo `013a04684e76` actif (lundi 9h, prochaine exécution 2026-08-17). Procédure de progression documentée dans `docs/cgpa/07-devsecops/procedure-progression-dmarc-lot4.md` : Phase 0 `p=none` → Phase 2 `p=quarantine; pct=25` (J+30, estimé 2026-09-12) → Phase 5 `p=reject` (J+88, estimé 2026-11-09). **RSV-EMAIL-NOREPLY-01-UNIDIR** : Gate Production documenté dans `docs/cgpa/09-production/gate-production-noreply-loyertracker-org-lot4-decision.md` — GO autorisé, exécution manuelle sur `loyertracker-prod-server` par opérateur habilité, validation par envoi réel + DKIM/SPF/DMARC. Contrainte d'unidirectionnalité permanente documentée. Aucune promotion Production sans Gate distinct.\n>\n
> **EP-17 Lot 3 — clôture des dettes `DD-611-03`, `DD-611-01` et `DD-EP17-05` (2026-08-13).** Branche `docs/lot3-tracabilite-uxr-modal`. **DD-611-03** : rapport de couverture de traçabilité `traceability-coverage-report.md` produit — 28 composants, 43 specs, 234 tests unitaires SUCCESS, gaps E2E/a11y/VR tracés comme réserves distinctes. **DD-611-01** : avis UX/UI Design Lead documenté dans `avis-ux-ui-design-lead-dd-611-01-uxr-001.md` — 5/5 critères PASS, UXR-001 rattaché au lot. **DD-EP17-05** : `lt-confirm-dialog` + `lt-toast` câblés au premier appelant métier `NotificationsPreferencesComponent` — migration du modal inline ad hoc, 6/6 exigences `DDS-LT-005` couvertes, 234/234 tests SUCCESS. Les trois dettes sont **CLOSE** dans le *Design Debt Register*. Aucune promotion Staging ou Production autorisée.\n>\n> **EP-17 Lot 2 — clôture des dettes `DD-EP17-02` et `DD-EP17-12` (2026-08-12).** La PR #462 (`2b2b82c`) a livré les états Frontend 403/404 et le parcours public d'acceptation d'invitation. Les digests immuables correspondants sont validés sur Staging : CI post-merge SUCCESS, `STG-ISOL-01` PASS, smoke réel **63 PASS / 0 FAIL**, puis recette humaine PO/CDO **PASS**, consignée et fusionnée par PR #464 (`20e2838`). Les deux dettes sont donc **CLOSE**, avec décision additive `docs/cgpa/design/decisions/cloture-dd-ep17-02-dd-ep17-12-lot2-2026-08-12.md`. La fermeture est bornée à Staging ; aucune promotion ou action Production n’est autorisée.
>
> **EP-17 Lot 2 — Gate Staging : GO / STAGING_DEPLOYED + recette humaine PASS (2026-08-12).** Après merge PR #462 (`2b2b82c`) et CI post-merge complète SUCCESS (build/tests, accessibilité E2E, sécurité, CodeQL, SBOM, signatures Cosign, provenance et attestations), les digests immuables API `sha256:4b7be8ec…` et Web `sha256:fd28b49f…` sont déployés sur `ai-test-server`. `STG-ISOL-01` PASS : namespace/réseau/volume dédiés, ports `18080/18443`, NPM mutualisé inchangé ; seuls `api` et `nginx` ont été recréés. Préflight backup PostgreSQL/globals hashé et vérifié ; Flyway 32/32, API/Web/Keycloak/PostgreSQL healthy, racine HTTPS et Actuator 200, smoke réel **63 PASS / 0 FAIL**, invitation→acceptation publique→JWT Gestionnaire et isolation cross-tenant PASS. Échafaudage Keycloak direct grant révoqué. **Recette humaine externe : PASS**, par Jordan Tshilombo Kabamba, PO/CDO, pour l’acceptation d’invitation et les états 403/404. Aucun changement Production autorisé ; la suite est l’instruction de la RC et des contrôles pré-Production distincts. Référence : `docs/cgpa/07-devsecops/gate-staging-ep17-lot2-parcours-utilisateur-critique.md`.
>
> **SEC-SMTP-01 / DD-EP17-14 — rotation SES et remédiation Staging validées (2026-08-13).** Une exposition historique d'identifiants SMTP dans le Compose a été traitée par rotation CDO des credentials, retrait des valeurs et fallbacks sensibles de tous les fichiers suivis, et injection locale uniquement (`.env` mode `0600`) sur Staging/Production. L'échappement Docker Compose des caractères `$` est contrôlé. En Staging : écriture atomique Keycloak 24 de `smtpServer`, relecture des clés runtime PASS, anti-énumération existant/inexistant `HTTP 200` avec message générique identique, et absence d'erreur SMTP dans les logs. La branche `docs/gate-production-smtp-keycloak-ep17` porte la remédiation ; **SMTP Production reste non activé** jusqu'à CI verte, fusion humaine et Gate Production séparé.
>
> **Lot 1 / DD-EP17-14 — SMTP Keycloak : dossier Production prêt pour décision CDO (2026-08-12).** Le GO Staging reste acquis (PR #456, parcours PO/CDO et anti-énumération PASS). Le mécanisme Production dédié et son rollback `smtpServer` sont mergés par PR #460 (`8d7f651`), sans rebuild ni déploiement. Le préflight Production, `CHECK-REL-01` et `CHECK-OPS-01` dédiés confirment le **PASS sous réserve opérationnelle** : backup PostgreSQL/globals hashé et `pg_restore --list` validé, services healthy, lock release cohérent, observabilité active. Restent obligatoires : fenêtre UTC, synchronisation non destructive de l'hôte, secret SMTP hors dépôt, test live et hypercare, puis décision CDO explicite `GO / PRODUCTION_READY`. Aucun changement Production autorisé par cette documentation. Références : `docs/cgpa/09-production/check-rel-01-dd-ep17-14-smtp-keycloak.md`, `check-ops-01-dd-ep17-14-smtp-keycloak.md`.
>
> **EP-18 — déploiement Staging `noreply@loyertracker.org` + surveillance DMARC hebdomadaire (2026-08-12).** Gate Staging distinct exécuté : `RESEND_FROM_EMAIL=noreply@loyertracker.org`, `RESEND_EMAIL_ENABLED=true` sur `ai-test-server`. Envoi réel validé (Resend ID `e7673f9b`). Décision complète : `docs/cgpa/07-devsecops/gate-staging-noreply-loyertracker-org-ep18-decision.md`. Surveillance DMARC hebdomadaire activée via cron (lundi 9h, job `013a04684e76`) : vérifie `s3://loyertracker-inbound-mail/dmarc/`, détecte les nouveaux rapports agrégés, décompresse et analyse SPF/DKIM. Prochaine exécution : 2026-08-17. Aucune promotion Production sans Gate distinct.

> **EP-18 — instruction consolidée `RSV-DMARC-02` + `RSV-EMAIL-NOREPLY-01` (2026-08-12).** Les deux réserves EP-18 restantes sont instruites dans `docs/cgpa/design/decisions/instruction-rsv-dmarc-02-email-noreply-01-ep18.md`. **`RSV-DMARC-02`** : `p=none` maintenu, aucun rapport DMARC agrégé réel reçu à date (domaine créé le 2026-08-10, les rapports sont émis une fois par jour par les fournisseurs). Plan : 30 jours d'observation après premier rapport → `p=quarantine; pct=25` → `p=quarantine; pct=100` → `p=reject`. Surveillance hebdomadaire du bucket S3 `loyertracker-inbound-mail/dmarc/`. **`RSV-EMAIL-NOREPLY-01`** : partiellement levée — le déploiement de `noreply@loyertracker.org` comme expéditeur est autorisé (Staging d'abord, puis Production via Gate distinct). La réserve est renommée `RSV-EMAIL-NOREPLY-01-UNIDIR` : l'unidirectionnalité persiste comme contrainte permanente (pas de boîte de réception, pied de page « ne pas répondre » requis). Aucun changement DNS, aucune modification de politique DMARC, aucun déploiement Production sans Gate distinct.

> **EP-17 / Lot A — fusion PR #450, `DD-611-02` CLOSE, CHECK-DESIGN-01 et baseline T0 intégrés (2026-08-12).** Après merge de la PR #450 (`ac44c6e`) et CI post-merge verte, le Lot A documentaire EP-17 / Gate 04A Lot 5 est consolidé sur `main` : `CHECK-DESIGN-01` Lot 5 (matrice design 8 contrôles), baseline visuelle post-pilote T0 (4 captures 390/640px avec hashes SHA-256), et levée bornée `DD-611-02` (DDS/DSG/inventaire composants). `DD-611-02` passe à **Close (2026-08-12)** dans le *Design Debt Register*. La preuve TLS stricte `RES-VR-04` (`a59f031`) est incluse. **Limites préservées** : `DD-611-03` (traçabilité Story→preuve) reste ouverte — Lot C requis ; `DD-EP17-10` (états d’erreur explicites) reste ouverte — Lot B requis ; Gate 04A global n’est pas déclaré clos ; aucune promotion Staging, Production, migration ou code applicatif autorisée.

> **EP-17 / US-137 — clôture technique CHECK-RESPONSIVE-01 : GO technique borné (2026-08-11).** Après merge de la PR #447 (`e1c72d7`) et CI post-merge verte (CI, CodeQL, Registry Policy, CGPA Framework Audit), la preuve Playwright authentifiée TLS stricte est **20/20 PASS** aux viewports 360/390/640/1024px, sans skip : routes exactes, axe, overflow, cibles 44px, reflow et reduced-motion. Le seed non sensible est **14 PASS / 0 FAIL** et restaure `directAccessGrants=false`. Jordan Tshilombo Kabamba, PO/CDO, valide la matrice manuelle QA/UX (clavier/focus, zoom 200%, reflow, reduced-motion, Invitation, Notifications et information financière). `RES-01→04`, `RES-RESP-PROOF-01`, `RES-DATA-01` et `RES-A11Y-ANGULAR-01` sont levées pour le périmètre couvert, conformément aux addenda de `check-responsive-01-ep17-us137.md` et `reserve-closure-a11y-responsive-data-ep17.md`. **Limite préservée :** ce GO technique ne statue pas le Gate 04A global, la baseline/régression visuelle, Staging ou Production ; aucune promotion n’est autorisée.

> **EP-18 — réception mail `loyertracker.org` opérationnelle, `RSV-DMARC-01` LEVÉE (2026-08-10).** La réception a été montée entièrement dans le compte AWS du projet (`381492172662`, région `eu-west-1`), **sans aucun tiers dans le flux mail** et sans coût récurrent fixe : identité SES `loyertracker.org` (`VerificationStatus: Success`) vérifiée par TXT `_amazonses.loyertracker.org`, MX racine `10 inbound-smtp.eu-west-1.amazonaws.com` (TTL `3600`), bucket `loyertracker-inbound-mail` avec accès public bloqué et policy restreinte à `ses.amazonaws.com` conditionnée par `AWS:SourceAccount` et `AWS:SourceArn`, rule set actif `loyertracker-inbound` et règle `dmarc-reports` dépotant sous le préfixe `dmarc/`. Rétention bornée par le cycle de vie S3 `expire-inbound-mail-180j` (expiration 180 jours, uploads incomplets abandonnés à 7 jours). **Preuve de bout en bout** : envoi réel vers `dmarc@loyertracker.org` (`MessageId 0102019fecac261e-…`), objet récupéré depuis S3, en-têtes conformes (`To: dmarc@loyertracker.org`, `Received-SPF: pass`). En conséquence, **`RSV-DMARC-01` est levée** : la directive `rua=mailto:dmarc@loyertracker.org;` est désormais opérationnelle. **Réserves maintenues** : `RSV-DMARC-02` — `p=none` reste purement observatoire, le durcissement vers `quarantine` puis `reject` suppose des rapports réels effectivement reçus et analysés, ce que la seule levée de `RSV-DMARC-01` ne fournit pas ; `RSV-EMAIL-NOREPLY-01` — seule `dmarc@` est acceptée, `noreply@loyertracker.org` demeure volontairement non recevable et le canal reste unidirectionnel. Limite connue : les rapports arrivent en MIME brut avec XML agrégé compressé, non lisibles sans traitement dédié, non fourni à ce stade. Point d'exploitation : le rule set actif est un réglage de compte et de région — aucun n'était actif en `eu-west-1` auparavant, toute activation concurrente désactiverait celui-ci. Aucun code, aucune migration, aucun secret, aucun Staging ni Production modifié ; `RESEND_FROM_EMAIL=onboarding@resend.dev` reste la valeur en vigueur en Production. Référence : `docs/cgpa/07-devsecops/runbook-resend.md` §9.5.

> **EP-18 — domaine d'envoi `loyertracker.org` vérifié côté Resend : capacité de notification e-mail depuis un domaine du projet (2026-08-10).** Le domaine `loyertracker.org` a été enregistré (compte AWS `381492172662`, registrar Route 53) et sa zone publique `Z0352366IY9T9FQ9R2JD` porte désormais les enregistrements d'authentification e-mail, tous confirmés par résolution publique : DKIM `resend._domainkey` (RSA 1024 bits, chaîne TXT unique), SPF `send` (MX `10 feedback-smtp.eu-west-1.amazonses.com` + TXT `v=spf1 include:amazonses.com ~all`) et DMARC `_dmarc` (`v=DMARC1; p=none; rua=mailto:dmarc@loyertracker.org;`), TTL `3600` s, délégation registrar alignée sur les NS de la zone. Le statut **Verified** dans l'interface Resend est **déclaré par le PO** le 2026-08-10 et n'a pas été rejoué par contrôle automatisé. **Capacité ouverte** : l'expéditeur du canal EMAIL peut désormais être une adresse `@loyertracker.org` authentifiée DKIM/SPF alignés, sans dépendre du domaine partagé du fournisseur. **Limites explicites** : la Production reste inchangée et conserve `RESEND_FROM_EMAIL=onboarding@resend.dev` (activation du 2026-08-06T00:21:46Z) ; tout changement d'expéditeur est une modification de configuration Production soumise à un Gate distinct ; l'allowlist de destinataires de test et le budget mensuel restent applicables. **Adresse expéditrice retenue (décision PO, 2026-08-10) : `noreply@loyertracker.org`** — valeur arrêtée mais **non déployée**, effective seulement au terme d'un Gate distinct ; le nom affiché (`RESEND_FROM_NAME`) et la cohérence de marque entre `loyertracker.org` et `loyerpro.org` restent à trancher. Ce domaine est **distinct de `loyerpro.org`** (zone `Z073040513ST6OIJ46NHK`), qui porte le produit (`loyertracker.loyerpro.org`, `loyertracker.staging.loyerpro.org`, URI de redirection Keycloak). Réserves ouvertes : `RSV-DMARC-01` — les rapports agrégés ne sont pas délivrés, `rua=` visant une boîte inexistante faute de MX racine ; `RSV-DMARC-02` — politique `p=none` purement observatoire, durcissement conditionné à la levée de `RSV-DMARC-01` ; `RSV-EMAIL-NOREPLY-01` — faute de MX racine, `noreply@loyertracker.org` n'est pas une boîte : toute réponse d'un destinataire est perdue sans notification, et les webhooks Resend n'étant pas activés (reportés EP-19), ni réponses ni rebonds ne sont observables. Aucun code, aucune migration, aucun secret, aucun Staging ni Production modifié par cette entrée. Référence : `docs/cgpa/07-devsecops/runbook-resend.md` §9.

> **EP-17 / US-136 — audit accessibilité Lot 5 : 6/6 flux Keycloak PASS, RSV-EP17-US136-A11Y-01 levée, clôture toujours NO GO (2026-08-11).** Les PR #437 et #438 ont validé la CI complète, dont `Accessibilité E2E (Playwright + axe)` : stack Docker isolée avec Mailpit `v1.25`, TLS `localhost` de confiance sans bypass, audit runtime des six flux Keycloak **6 passed**, artifacts GitHub `9064684806` + `9095712663`. Les flux couverts : login, mot de passe oublié, réinitialisation (action-token via Mailpit), session expirée, accès refusé, logout. `RSV-EP17-US136-A11Y-01` est **levée** pour les six flux Keycloak. `RSV-EP17-US136-A11Y-02` (parcours Angular authentifié) et `RSV-EP17-US136-A11Y-03` (matrice manuelle clavier/focus/zoom/reflow/reduced-motion) restent ouvertes. Aucun Gate historique, Staging, Production, realm, URI de redirection ou secret persistant modifié. Références : `docs/cgpa/checklists/check-accessibility-01-ep17-us136.md`, PR #437, PR #438.

> **EP-17 / US-137 — CHECK-RESPONSIVE-01 : GO sous réserve (2026-08-11, rectifié par preuves US-138).** Contrôle responsive exécuté via navigateur direct, puis captures Chromium déterministes aux viewports 640px et 390px. **Écrans réellement accessibles et PASS** : Keycloak Login et Dashboard Bailleur (`overflowX=0` pour Angular). Les routes `/gestionnaire`, `/bailleur/profil`, `/bailleur/locataires`, `/bailleur/gestionnaires` et leurs détail redirigent vers `/bailleur` dans le build exécuté : les captures antérieurement attribuées à Gestionnaire/Profil étaient donc des captures du Dashboard Bailleur et ne constituent pas une preuve pour ces écrans. Écrans intégrés (Invitation, Notifications) non rendus faute de données (0 gestionnaire, 0 bien). **Réserves** : RES-01 (routes/build Angular à corriger), RES-02 (données de test nécessaires), RES-03 (R03 info financière non testable), RES-04 (R04 touch targets vérifié visuellement, accepté). Référence : `docs/cgpa/checklists/check-responsive-01-ep17-us137.md` et preuves `docs/cgpa/evidence/ep17-us138/`.

> **EP-17 / US-138 — CHECK-VISUAL-REGRESSION-01 : GO sous réserve (2026-08-11).** Visual Review exécutée par captures Chromium déterministes aux viewports 640px et 390px ; 8 PNG versionnés sous `docs/cgpa/evidence/ep17-us138/`. Dashboard Bailleur : **PASS** (`overflowX=0`, aucune image cassée, empilement mobile lisible). Login Keycloak : thème cohérent avec l'Angular, mais débordement horizontal mesuré à **10px** aux deux viewports (`RES-VR-04`). `/gestionnaire` et `/bailleur/profil` redirigent vers `/bailleur`, donc aucune validation visuelle valide de ces écrans ; baseline US-127 avant pilote inexistante, comparaison avant/après impossible. `DD-611-04` passe à **En traitement**, sans clôture ni approbation implicite de dette. Référence : `docs/cgpa/checklists/check-visual-regression-01-ep17-us138.md`.

> **EP-17 / US-139 — documentation post-pilote : candidat soumis à revue (2026-08-11).** `DSG-001.md` est incrémenté en **v0.3.0 — Candidat post-pilote** ; `component-inventory-loyertracker.md` reflète désormais les **26 composants Angular** réellement présents (18 applicatifs/métier, 8 transverses `lt-*`/Toast) et le thème Keycloak réel. `traceability-ui-loyertracker.md` relie les preuves EP-17/US-136→138 et ne conserve aucun « À définir » sur les lignes pilote EP-17/US-132→140 ni EP-16/US-125. État source et comportement runtime sont volontairement dissociés : les routes existent dans `app.routes.ts`, mais plusieurs redirigent vers `/bailleur` dans le build contrôlé. Le thème Keycloak associe CSS et surcharges FreeMarker ciblées pour l’accessibilité ; il n’est donc pas CSS-only. Réserves préservées : build/runtime Angular, données de test insuffisantes, baseline US-127 absente, `RES-VR-04` (overflow Keycloak 10px) et parcours Angular a11y non prouvé. Aucun code applicatif, environnement, migration, Staging ni Production modifié.

> **EP-17 / US-140 — réinstruction Gate 04A Lot 5 soumise au PO/CDO (2026-08-11).** Une réinstruction additive préserve intégralement la décision historique et consolide US-136→139 ainsi que deux avis indépendants Governance/QA et Frontend/Design QA. Recommandation unanime : **NO GO pour clôture complète** ; seul un **GO sous réserve documentaire**, explicitement borné et accepté par le PO/CDO, peut être envisagé. Bloquants/réserves : `CHECK-DESIGN-01` absent ; preuves responsive US-137 non versionnées ; divergence routes source/build-runtime ; données de test absentes ; parcours Angular a11y/matrice manuelle non prouvés ; baseline US-127 absente ; overflow Keycloak 10px. Aucun développement, déploiement, promotion ni levée de réserve autorisé. Référence : `docs/cgpa/design/decisions/gate-04A-ep17-lot5-us140-reinstruction.md`.

> **EP-17 / US-141 — Gate Staging du pilote : GO / STAGING_DEPLOYED (2026-08-11).** Instance Staging `i-0156f2672ee800e5a` redémarrée, dépôt synchronisé `d19c4fe` → `0ed7525` (fast-forward documentaire, aucune reconstruction d'image). `STG-ISOL-01` PASS : 9 conteneurs, namespace `loyertracker-staging-*`, réseau/volume dédiés, NPM mutualisé inchangé, ports `18080`/`18443` conservés, aucun impact interprojet. Smoke réel **63 PASS / 0 FAIL** : JWT Keycloak réel, issuer portless, ReBAC, isolation cross-tenant, RGPD, audit, ports internes. Images inchangées depuis US-125 (`sha-d19c4fea`). Réserves Lot 5 restantes (`CHECK-RESPONSIVE-01`, `CHECK-DESIGN-01`, `DD-EP17-10`, `DD-611-02/03`) ne bloquent pas ce Gate. Aucune promotion Production autorisée. Décision : `docs/cgpa/07-devsecops/gate-staging-us141-decision.md`.

> **Décision PO/CDO — réconciliation backlog points 1 et 2 — VALIDÉE (2026-08-09).** Jordan Tshilombo valide : (1) les IDs source Patrimoine `US-73→US-75` sont absorbés par `US-83→US-85`, tandis que `US-76→US-79` et `US-86→US-89` sont classés non attribués/réservés ou annulés avant attribution ; (2) la collision EP-17/EP-18 `US-135→US-140` est résolue par migration documentaire additive avec alias historiques et identifiants qualifiés obligatoires `EP-17/US-xxx` et `EP-18/US-xxx`. L’historique, les commits, PR, Gates et releases ne sont pas réécrits. Cette validation autorise uniquement la synchronisation documentaire ; aucun code, migration, Staging, Production, provider ou EP-19 n’est autorisé. Référence : `docs/cgpa/06-planification-agile/decision-proposee-reconciliation-backlog-points-1-2-2026-08-09.md`, PR #417.

> **Décision de reprise Dev post-US-125 — `Resume Approved with Reservations` (2026-08-09T06:27:44Z).** Audit du backlog EP-16 effectué : US-124/126 sont couvertes par `1.15.0`, US-125 par `1.17.0-rc.1`; aucun reliquat fonctionnel EP-16 n’est prouvé. Branche dédiée `audit/sprint-remainder-after-us125` créée pour la réconciliation documentaire et le cadrage du prochain item. Aucun merge applicatif, migration, changement partagé, promotion ou déploiement autorisé pendant l’hypercare. Artefact : `docs/cgpa/06-planification-agile/sprint-remainder-audit-after-us125.md`.
...
> **Gate Staging US-125 — GO / STAGING_DEPLOYED (2026-08-08T22:09:25Z).** Validation humaine explicite reçue. Artefacts immuables API/Web promus sur `ai-test-server` depuis le commit `d19c4fea850263c5bfbb92fcb288dd7bd2e56e2a`. STG-ISOL-01 PASS avant/après ; backup PostgreSQL vérifié ; Flyway 32/32 ; smoke réel **63 PASS / 0 FAIL** ; health/API, Actuator, JWT, ReBAC, isolation cross-tenant, RGPD et ports internes PASS. Seuls `api` et `nginx` Staging ont été recréés. Production reste inchangée et interdite par ce Gate. Décision : `docs/cgpa/07-devsecops/gate-staging-us125-decision.md`.

> **Levée technique des réserves US-125 — CI distante verte, Gate Staging à instruire (2026-08-08).** `RSV-MIG-611-04` est documentée par l'addendum DAT/OpenAPI `docs/cgpa/05-architecture-conception/addendum-dat-openapi-us125-notifications.md`. `RSV-US125-FE-01` est couverte par le contrôleur/API intégré, les règles JWT/RLS/ReBAC et le test d'intégration `NotificationFondationIntegrationTest`. `RSV-US125-A11Y-01` est couverte techniquement par les preuves ChromeHeadless US-125 : focus initial dans le dialogue, fermeture `Escape`, restitution du focus, boucle Tab, `aria-modal`/labels et touch targets 44 px. `RSV-US125-RESP-01` est couverte par une exécution ChromeHeadless aux quatre largeurs CGPA (`360px`, `390px`, `640px`, `1024px`) : **44/44 assertions PASS**, absence de débordement horizontal et empilement mobile confirmé. PR #398 fusionnée dans `main` avec le commit `e27034a2b0496b894d1e404567303ba77b88656a`. Réserves techniquement levées, CI distante verte, aucun Staging, Production, secret, provider ou migration supplémentaire exécuté.
...
> **Addendum Gate 05 US-125 — migration de sécurité autorisée, décision PO/CDO (2026-08-06T13:08:39+01:00).** Le PO/CDO valide l'option S1 : préférence globale du Gestionnaire séparée des préférences tenant-scopées, plus provenance `bien_id` sur les événements et fonction PostgreSQL `SECURITY DEFINER` fail-closed pour l'historique ReBAC. Cette décision autorise exclusivement une migration Flyway additive, les services/API et les tests associés nécessaires à US-125 ; elle ne modifie ni les providers ni leurs flags. Le plan détaillé est ajouté à `gate-05-us125-backend-planification.md` §10. Restent interdits : Staging, Production, smoke destructif, secret, activation Twilio/SMS/WhatsApp/Resend et EP-19.

> **Gate 05 US-125 Backend — planification approuvée (2026-08-06T09:43:23Z).** Après merge de la PR #386 (`a3354c1`), l'interface Frontend US-125 est intégrée sur `main`, mais son contrat HTTP cible n'est pas livré côté Backend : `GET/PUT /api/notifications/preferences/current`, `POST /api/notifications/preferences/current/unsubscribe` et `GET /api/notifications/history` restent à exposer de manière sécurisée. Le dossier `docs/cgpa/06-planification-agile/gate-05-us125-backend-planification.md` régularise cet état et porte le plan Backend : autorisation côté serveur, RLS/ReBAC, DTO minimaux, masquage de l'historique, tests 401/403/cross-tenant/périmètre Gestionnaire et intégration Frontend. La validation humaine PO/CDO est tracée par la fusion de cette décision ; l'implémentation reste limitée par l'addendum de sécurité ci-dessus. Toujours exclus hors cette exception : Staging, Production, secret, activation Twilio/SMS/WhatsApp/Resend et EP-19.

> **Gate 04A US-125 — GO sous réserve documentaire (2026-08-06T06:40:53Z).** Après merge PR #383 (`b78429f`) et feu vert PO/CDO, l'instruction Gate 04A du périmètre EP-16 Sprint N+2 Lot B / **US-125** est produite : UI specs, responsive, accessibilité, revue design, tokens, inventaire composants, dette design et architecture Frontend sont consolidés. Décision : `docs/cgpa/design/decisions/gate-04A-decision-ep16-us125.md`. Portée autorisée uniquement après merge de la PR documentaire : démarrer une branche de développement **Frontend US-125** strictement conforme aux réserves `RSV-US125-A11Y-01`, `RSV-US125-RESP-01`, `RSV-US125-FE-01`. Toujours interdit : backend, migration, Staging, Production, secret, activation Twilio/SMS/WhatsApp et EP-19.

> **GO PO/CDO — cadrage US-125 / Gates 02A-04A autorisé, sans codage (2026-08-06).** Après clôture CDO de `1.16.0`, le PO choisit le sprint backlog produit suivant : EP-16 Sprint N+2 Lot B / **US-125** (interface de préférences et historique notifications). Décision limitée au cadrage UX/UI et à l'instruction Gate 04A : Gate 02A US-125 est déjà statué GO sous réserve, UXR-001 et DDS-LT-002→005 sont renseignés/acceptés, DSG-001 est instancié ; Gate 04A désormais instruit par la décision dédiée ci-dessus. Aucun codage Frontend, backend, migration, Staging, Production, secret, activation Twilio/SMS/WhatsApp ou EP-19 n'est autorisé par cette décision de cadrage. Décision : `docs/cgpa/06-planification-agile/cadrage-us125-gates-02a-04a.md`.

> **RELEASE `1.16.0` CLÔTURÉE — CDO GO (2026-08-06T05:59:59Z).** Après merge PR #381 (`52683c5`) et resynchronisation de l'hôte Production sur `main`, la release EP-18 est officiellement clôturée : `PRODUCTION_DEPLOYED` confirmé, hypercare T0/T+12/T+24 accepté PO sans incident bloquant, Resend Production activé (`RESEND_EMAIL_ENABLED=true`, `NOTIFICATIONS_EXTERNAL_ENABLED=true`, `NOTIFICATION_DRY_RUN=false`, budget `100`, `RESEND_FROM_EMAIL=onboarding@resend.dev`), `check-release-state.sh --host` cohérent, Flyway `31/31`, API/Web/PostgreSQL/Keycloak healthy, `/healthz` et racine publique `200`, `notification_outbox/delivery=0/0`. Cette clôture n'autorise aucun nouveau développement, déploiement, smoke destructif, migration, activation Twilio ou démarrage EP-19. Décision : `docs/cgpa/09-production/cloture-release-v1.16.0.md`.

> **Activation Resend Production EP-18 — RESEND_PRODUCTION_ENABLED (2026-08-06T00:21:46Z).** Sur instruction PO explicite « Active le resend », Resend a été activé en Production avec `RESEND_EMAIL_ENABLED=true`, `RESEND_FROM_EMAIL=onboarding@resend.dev`, `NOTIFICATIONS_EXTERNAL_ENABLED=true`, `NOTIFICATION_DRY_RUN=false` et budget conservateur `NOTIFICATION_BUDGET_MENSUEL_MAX=100` (`seuil=0.8`). Correction opérationnelle associée : `docker-compose.yml` transmet désormais `NOTIFICATION_BUDGET_MENSUEL_MAX` et `NOTIFICATION_BUDGET_SEUIL_ALERTE` au conteneur API ; sinon l'application restait bloquée à `0/0`. Recréation ciblée `api` uniquement ; `nginx`/`postgres`/`keycloak` inchangés ; `/healthz` et racine publique `200`, release lock hôte cohérent, Flyway `31/31`, outbox/delivery `0/0`, budget métrique `0/100`. `NotificationKillSwitchFerme` reste temporairement firing par fenêtre `increase(...[30m])`, à recontrôler après expiration. Rapport : `docs/cgpa/09-production/activation-resend-production-ep18-report.md`.

> **Décision PO Hypercare EP-18 — checkpoints anticipés acceptés (2026-08-05T22:10:00Z).** Le PO accepte explicitement les pré-checks T+12 et T+24 exécutés immédiatement comme valeur probante suffisante. Gouvernance mise à jour : T0 PASS, T+12 anticipé **ACCEPTÉ PO / PASS hypercare**, T+24 anticipé **ACCEPTÉ PO / PASS hypercare** ; hypercare EP-18 considéré terminé sans incident bloquant. Les WARN fenêtre restent historiques mais ne bloquent plus la conclusion d'hypercare. Limites préservées : Resend reste désactivé, aucun canal externe activé, aucune migration/smoke destructif/suppression autorisée ; la clôture CDO `1.16.0` reste une décision distincte. Rapport : `docs/cgpa/09-production/plan-etape-hypercare-v1.16.0.md`.

> **Hypercare EP-18 pré-check T+24 — PASS technique / WARN fenêtre (2026-08-05T21:57:20Z).** Exécuté immédiatement sur instruction PO explicite, ~21h28 avant la cible `2026-08-06T19:25:04Z ± 30 min`. Contrôles Production lecture seule verts : `check-release-state.sh --host` cohérent, API/Web/PostgreSQL/Keycloak healthy `RestartCount=0`, Flyway `31/31`, `/healthz` et racine publique `200`, baseline métier inchangée, `notification_event/outbox/delivery/template/preference=34/0/0/4/0`, `directAccessGrants=false`, `bailleur-test=false`, Resend désactivé (`RESEND_EMAIL_ENABLED=false`, `RESEND_FROM_EMAIL` absent). Alertes requalifiées non bloquantes à l'instant du pré-check : `NotificationKillSwitchFerme` attendue tant que les canaux externes restent fermés ; `BackupHeartbeatMissing` encore attendue car le cron backup `02:15 UTC` n'avait pas encore rejoué depuis le redémarrage/Pushgateway volatil. Rapport : `docs/cgpa/09-production/plan-etape-hypercare-v1.16.0.md`. Le checkpoint officiel de fenêtre T+24 reste à interpréter selon décision PO/CDO.

> **Hypercare EP-18 pré-check T+12 — PASS technique / WARN fenêtre (2026-08-05T21:05:52Z).** Le pré-check T+12 a été exécuté immédiatement sur instruction PO explicite avant la fenêtre cible `2026-08-06T07:25:04Z ± 30 min`. Contrôles Production lecture seule verts : `check-release-state.sh --host` cohérent, API/Web/PostgreSQL/Keycloak healthy `RestartCount=0`, Flyway `31/31`, racine publique `200` (``/healthz` répond `200`), baseline métier inchangée, `notification_event/outbox/delivery/template/preference=34/0/0/4/0`, `directAccessGrants=false`, `bailleur-test=false`, Resend désactivé (`RESEND_EMAIL_ENABLED=false`, `RESEND_FROM_EMAIL` absent). Alertes requalifiées non bloquantes à l'instant du pré-check : `NotificationKillSwitchFerme` attendue tant que les canaux externes restent fermés ; `BackupHeartbeatMissing` encore attendue car le cron backup `02:15 UTC` n'avait pas encore rejoué depuis le redémarrage/Pushgateway volatil. Rapport : `docs/cgpa/09-production/plan-etape-hypercare-v1.16.0.md`. Le checkpoint officiel de fenêtre T+12 reste à interpréter selon décision PO/CDO.

> **Hypercare EP-18 T0 — PASS (2026-08-05T19:57:29Z).** Après merge PR #375 et resync hôte sur `a239e0a`, checkpoint T0 lecture seule vert : `check-release-state.sh --host` cohérent, API/Web/PostgreSQL/Keycloak healthy `RestartCount=0`, Flyway `31/31`, `/healthz` et racine publique `200`, baseline métier inchangée, `notification_event/outbox/delivery/template/preference=34/0/0/4/0`, `directAccessGrants=false`, `bailleur-test=false`, Resend désactivé. Alertes actives qualifiées non bloquantes : `NotificationKillSwitchFerme` attendu tant que les canaux externes restent fermés ; `BackupHeartbeatMissing` lié au Pushgateway volatil/cron backup pas encore rejoué depuis redémarrage. Rapport : `docs/cgpa/09-production/plan-etape-hypercare-v1.16.0.md`.

> **Validation finale Production EP-18 — PASS / `PRODUCTION_DEPLOYED` (2026-08-05).** Après merge PR #374 et resync hôte sur `9be44e8`, smoke Production canonique exécuté avec autorisation PO : RUN_ID `1785957904`, **65 PASS / 0 FAIL**, Flyway `31/31`, digests API/Web conformes, isolation cross-tenant/RGPD/alertes/audit/ports/JWT réels validés. Nettoyage transactionnel complet : 47 lignes DB supprimées, comptes Keycloak smoke supprimés, résidus RUN_ID `0`; `directAccessGrants=false`, `bailleur-test=false`. Resend reste désactivé (`RESEND_EMAIL_ENABLED=false`) et `notification_outbox/delivery=0/0` final. Rapport : `docs/cgpa/09-production/validation-finale-ep18-notifications-email-resend-report.md`.
...
## 2026-08-05 — Clôture d’intégration repository EP-18 après merge PR #368

**Instruction PO/CDO validée** : exécuter le next step CGPA non technique après merge EP-18 —
clôturer l’intégration repository avant toute instruction Staging/Production.

**État réel vérifié avant décision** :

- branche de travail dédiée créée depuis `origin/main` : `docs/cloture-integration-ep18` ;
- PR #368 : **MERGED** le 2026-08-05T13:28:32Z ;
- merge commit `main` : `8c9f1e4aaa2a57946af841bbccd052f38fd471be` ;
- head intégré : `e3f9d294a41113dd6b4ec1cbfc06b839ed92b7e2` ;
- checks PR verts : CI, Backend, Frontend, Sécurité Gitleaks/SCA/Trivy, Docker build/scan/SBOM,
  CodeQL Java/Kotlin, CodeQL JavaScript/TypeScript, Registry Policy, CGPA Framework Audit ;
- checks post-merge `main` verts sur `8c9f1e4` : CI `31010371683`, CodeQL `31010371695`, Registry
  Policy `31010371646`, CGPA Framework Audit `31010371651` ;
- Production inchangée : release `1.15.0`, tag `sha-ac374193`, Flyway Production `29/29`,
  `PRODUCTION_DEPLOYED_AT=2026-07-30T13:20:00Z` ;
- dépôt à jour avec EP-18 : `FLYWAY_EXPECTED_REPO=31`, `FLYWAY_EXPECTED_PROD=29`.

**Décision** : **GO sous réserve — clôture d’intégration repository EP-18**.

Le périmètre EP-18 intégré couvre le Sprint A (socle EMAIL/Resend), le Sprint B (invitation
Gestionnaire par e-mail) et le Sprint C (webhooks Resend). Cette décision ferme le cycle
PR/intégration uniquement. Elle ne vaut ni Gate Staging, ni Gate Production, ni activation Resend.

**Réserves maintenues** :

- `RSV-EP18-06` ouverte : schéma de signature webhook Resend/Svix non vérifié contre trafic réel ;
- aucune promotion Staging EP-18 instruite ;
- aucune activation Resend réelle autorisée ;
- aucune Production EP-18 autorisée.

**Artefact de décision** :
`docs/cgpa/06-planification-agile/cloture-integration-ep18-notifications-email-resend.md`.

**Prochaine décision autorisée** : instruire un **Gate Staging EP-18** distinct, avec `CHECK-CICD-01`,
...
> **Décision PO/CDO — registre Bugs et matrice normative — VALIDÉE (2026-08-09).** Jordan Tshilombo approuve le registre `docs/cgpa/06-planification-agile/registre-bugs-defauts-2026-08-09.md`, la matrice `docs/cgpa/06-planification-agile/matrice-tracabilite-story-bug-2026-08-09.md`, le triage initial P0/P1/P2 et le statut `GO documentaire sous gel applicatif / NO GO fonctionnel` pendant l’hypercare `v1.17.0-rc.1`. Les candidats `BUG-CAND-*`, `DOC-CAND-*` et `DEBT-CAND-*` restent à qualifier individuellement. Cette validation n’autorise aucun code, migration, Staging, Production, provider, secret, EP-19 ou observabilité critique. Référence : `docs/cgpa/06-planification-agile/decision-proposee-bugs-matrice-2026-08-09.md`, PR #419.
> **Règle PO/CDO — suppression/dépréciation d’endpoint — VALIDÉE (2026-08-09).** Avant toute suppression future, les neuf contrôles suivants sont obligatoires : recherche Angular complète ; wrappers/appels indirects ; smoke/tests/backend ; NGINX, batch, monitoring et providers ; consommateurs externes ; logs d’usage disponibles ; décision PO/CDO ; PR dédiée avec non-régression ; plan de rollback. L’audit Backend↔Frontend est statique et n’a pas interrogé les logs runtime Production : l’absence d’usage Angular détecté ne prouve pas l’absence de trafic. Aucun endpoint n’est supprimé par cette décision. Référence : `docs/cgpa/06-planification-agile/audit-backend-frontend-api-2026-08-09.md`.
