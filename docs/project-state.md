> **EP-17 / US-136 — audit accessibilité Lot 5 : NO GO technique pour clôture (2026-08-10T10:06:41Z).** Contrôles de non-régression Frontend verts (`npm run lint` PASS ; Karma ChromeHeadless **223/223 SUCCESS**), mais l’acceptation WCAG 2.2 AA n’est pas démontrée : navigation OIDC locale bloquée par le certificat non approuvé (`ERR_CERT_AUTHORITY_INVALID`) et URI de redirect du port local non autorisée ; diagnostic exploratoire axe-core sur le thème Keycloak : contraste `.instruction` **3,88:1** (< 4,5:1) et landmark principal absent du template hérité. Les essais d’outillage/dépendances et correctifs CSS/JS non autorisés ont été supprimés sur validation PO ; aucun changement produit, secret, Staging ou Production. Réserves `RSV-EP17-US136-A11Y-01→03` ouvertes. Preuves : `docs/cgpa/checklists/check-accessibility-01-ep17-us136.md`, `docs/cgpa/06-planification-agile/rapport-audit-accessibilite-ep17-us136-2026-08-10.md`.
