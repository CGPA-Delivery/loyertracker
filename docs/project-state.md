> **EP-20 — Paiement d’une échéance par garantie locative : cadrage factuel / NO GO applicatif (2026-08-13).** Audit sur `main` `0161a16d` : le flux US-95 existe (ledger `RETENUE_LOYER`, FK V21, audit, Outbox), mais une retenue après paiement classique remplace `montant_recu` et compare la retenue isolée au montant attendu ; le cas 400 + 600 sur 1 000 produit donc à tort `PARTIEL`. Quittance ADR-15, lien HMAC, Outbox/consentement/fallback/callback existent; restent à compléter le calcul cumulatif et la concurrence, le payload/rendu `GARANTIE_DEBITEE`, le document `PARTIEL`, la nomenclature PDF commune et l’UI de confirmation/résultat. EP-20/US-148→153 sont proposés sans collision (EP-19 préservé). **NO GO applicatif** jusqu’à approbation PO/CDO du Plan et de l’arbitrage reçu partiel distinct recommandé; aucun code, migration, Staging ou Production autorisé. Références : `docs/cgpa/06-planification-agile/plan-execution-ep20-garantie-echeance.md`, `rapport-cadrage-ep20-garantie-echeance-2026-08-13.md`.
