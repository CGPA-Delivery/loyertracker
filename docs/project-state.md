...
> **Gate 05 US-125 Backend — planification proposée, validation humaine requise (2026-08-06T09:23:18+01:00).** Après merge de la PR #385 (`b54303d`), l'interface Frontend US-125 est intégrée sur `main`, mais son contrat HTTP cible n'est pas livré côté Backend : `GET/PUT /api/notifications/preferences/current`, `POST /api/notifications/preferences/current/unsubscribe` et `GET /api/notifications/history` restent à exposer de manière sécurisée. Le dossier `docs/cgpa/06-planification-agile/gate-05-us125-backend-planification.md` régularise cet état et propose le plan Backend : autorisation côté serveur, RLS/ReBAC, DTO minimaux, masquage de l'historique, tests 401/403/cross-tenant/périmètre Gestionnaire et intégration Frontend. **Aucun codage Backend n'est autorisé par cette proposition tant qu'un GO humain PO/CDO explicite n'est pas tracé.** Toujours exclus : migration, Staging, Production, secret, activation Twilio/SMS/WhatsApp/Resend et EP-19.
