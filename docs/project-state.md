> **EP-20 — EP20-US04 : quittance certifiée après retenue, candidat à revue locale (2026-08-14).** Implémentation sur `feat/ep20-us04-quittance-recu-only` : une retenue qui solde le paiement n’émet automatiquement une quittance que si le profil documentaire est complet; une adresse bailleur absente ne bloque jamais l’écriture financière et laisse la quittance réémissible après correction. ADR-15 est prouvé au contrat HTTP : `PARTIEL` retourne `409` et persiste `0` quittance; `RECU` après retenue, avec profil complet, produit exactement `1` quittance `EMISE` liée au paiement. Tests ciblés, suites S03/quittances et `mvn -q verify` sont PASS avec Flyway 34. Commits : `57e0391`, `a8c555b`, `fde5965`. PR, CI et revue humaine restent requises; aucun provider, secret, envoi réel, Staging ou Production n’a été activé/exécuté.
