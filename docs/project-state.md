> **EP-20 — EP20-US05 : nomenclature PDF unifiée, candidat à revue locale (2026-08-14).** Les téléchargements authentifié et public servent désormais le même nom stable `quittance-certifiee-YYYY-MM.pdf`, dérivé de la période certifiée uniquement. La factory rejette tout format hors `YYYY-MM`, éliminant l’injection de chemin/PII. HMAC, intégrité PDF et `404` public indifférencié sont inchangés. Preuves : RED/GREEN factory, contrôleurs authentifié/public, `DocumentGenerationIntegrationTest`, `PublicQuittanceIntegrationTest`, `QuittanceFilenameFactoryTest` et `mvn -q verify` PASS sur Flyway 34. PR/CI/revue humaine restent requis; aucun provider, secret, envoi réel, Staging ou Production.
